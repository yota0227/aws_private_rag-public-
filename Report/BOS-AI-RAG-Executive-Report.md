...
| 8 | RAG v2 | RTL 파이프라인 분리 | RTL 전용 S3 + Parser Lambda + OpenSearch 인덱스 + IAM Explicit Deny | 📋 계획 | 1주 (구현) |
| 9 | RAG v2 | Claim DB + Ingestion | 지식 DB(DynamoDB 5 GSI) + 오염 방지 + 문서→Claim 변환 | 📋 계획 | 1주 (구현) |
| 10 | RAG v2 | MCP Bridge v2 + 검증 | search_archive, get_evidence, list_verified_claims + 8단계 Verification Pipeline | 📋 계획 | 1주 (구현) |
| 11 | RAG v2 | HDD 자동 생성 + 승인 | generate_hdd_section, publish_markdown + Human Review Gate | 📋 계획 | 1주 (구현+테스트) |
| 12 | RAG v2 | 교차 검증 + KPI | Cross-Check 파이프라인 + 운영 KPI 7종 + 통합 테스트 | 📋 계획 | 1주 (구현+테스트) |
| | **RTL Knowledge Graph** | | | | |
| 13 | RAG v3 | Neptune Graph DB + 관계 추출 | RTL 모듈 간 관계 자동 추출 → Neptune 노드/엣지 적재 + 3저장소 통합 질의 | 📋 계획 | 2주 (구현+테스트) |
...
2. 단순 "문서 검색 AI"가 아니라, 근거 기반 답변 + 교차 검증 + 오염 방지가 내장된 "검증 가능한 지식 운영 체계"입니다.

3. 엔지니어들이 현재 수동으로 수행하는 RTL 분석 → Memory Archive 작성 → HDD 문서 생성 → Cross-Check 작업을 시스템이 자동화하여, 설계 생산성을 향상시킵니다.

4. 전체 13개 마일스톤 중 7개(54%)가 완료되었으며, 나머지 6개는 약 7주 내 구현 완료 예정입니다. Phase 1~5 추가 비용은 월 $40 이내이며, Phase 6(Graph DB) 포함 시 월 $280 수준입니다.
