...
사내 네트워크와 AWS는 **VPN + Transit Gateway**로 연결됩니다:

```
사내 네트워크 (192.128.0.0/16)
    │
    │  IPsec VPN (암호화 터널 2개, 고가용성)
    │
    ▼
Transit Gateway (중앙 라우터)
    │
    ├──→ Private RAG VPC (10.10.0.0/16) ── 서울, RAG 전용
    │         └──→ VPC Peering ──→ Backend VPC (10.20.0.0/16) ── 버지니아
    │
    └──→ Logging VPC (10.200.0.0/16) ── 서울, 모니터링 전용
```
...
Obot은 MCP 프로토콜을 통해 RAG API를 호출하고, Bedrock KB가 관련 문서를 검색하여 Claude AI가 답변을 생성합니다.
