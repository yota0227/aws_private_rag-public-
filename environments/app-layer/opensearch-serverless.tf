...
resource "aws_security_group" "opensearch" {
  ...
  # Inbound Rules
  # Allow HTTPS from Lambda security group (to be created)
  ingress {
    description     = "HTTPS from Lambda"
    from_port       = 443
    to_port         = 443
    protocol        = "tcp"
    security_groups = ["sg-lambda-id"]  # Replace with actual Lambda SG ID
  }

  # Allow HTTPS from Bedrock KB security group (to be created)
  ingress {
    description     = "HTTPS from Bedrock Knowledge Base"
    from_port       = 443
    to_port         = 443
    protocol        = "tcp"
    security_groups = ["sg-bedrock-kb-id"]  # Replace with actual Bedrock KB SG ID
  }
...
resource "aws_opensearchserverless_security_policy" "encryption" {
  name        = "bos-ai-rag-vectors-prod-encryption"
  type        = "encryption"
  ...
  policy = jsonencode([
    {
      ...
      SourceVPCEs = [
        "vpce-opensearch-serverless-id"  # Replace with actual VPC endpoint ID
      ]
    }
  ])
...
resource "aws_opensearchserverless_vpc_endpoint" "main" {
  name               = "vpce-opensearch-serverless-seoul-prod"
...
resource "aws_opensearchserverless_access_policy" "data_access" {
  name        = "bos-ai-rag-vectors-prod-data-access"
  type        = "data"
