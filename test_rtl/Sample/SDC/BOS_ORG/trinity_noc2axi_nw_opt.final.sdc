...
global SDC_DIR
set ::DESIGN_NAME trinity_noc2axi_nw_opt
if {![info exists ::tt_global_is_mapped]} { set ::tt_global_is_mapped 1 }
if {![info exists ::DESIGN_NAME]} { set ::DESIGN_NAME {trinity_noc2axi_nw_opt} }
global DESIGN_NAME
...
set_ideal_network -no_propagate [get_port {i_axiclk}]
...
set my_clock [get_clocks -quiet { vir_NOCCLK }]
if { [sizeof_collection $my_clock]>0 && [sizeof_collection $my_ports]>0 } {
  puts {-I- setting output delay of 50% of NOCCLK on o_noc_reset_n}
  set_output_delay -max -clock $my_clock  [expr { 50 * $::NOCCLK_PERIOD / 100.0 }] $my_ports
} else {
  puts {-W- skipping output delay 50% of NOCCLK on o_noc_reset_n}
}
...
if {$::DESIGN_NAME eq "trinity_noc2axi_nw_opt"} {
  set cur_instance {}
  set_max_fanout 32 [current_design]
...
#    Generate collapsed synth reports
  ...
  # pathgroups moved to trinity_noc2axi_nw_opt.pathgroups.tcl
}
