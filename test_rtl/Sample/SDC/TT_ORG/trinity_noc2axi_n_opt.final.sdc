...
set my_clock [get_clocks -quiet { NOCCLK }]
if { [sizeof_collection $my_clock]>0 && [sizeof_collection $my_ports]>0 } {
  puts {-I- adding additional output delay of 60% of NOCCLK on *}
  set_output_delay -add_delay -max -clock $my_clock  [expr { 60 * $::NOCCLK_PERIOD / 100.0 }] $my_ports
} else {
  puts {-W- skipping output delay 60% of NOCCLK on *}
}

set my_ports [remove_from_collection [get_ports -quiet { * } -filter "direction=~in*"] $::my_in_clock_ports]
set my_clock [get_clocks -quiet { AXICLK }]
if { [sizeof_collection $my_clock]>0 && [sizeof_collection $my_ports]>0 } {
  puts {-I- adding additional input delay of 60% of AXICLK on *}
  set_input_delay -add_delay -max -clock $my_clock  [expr { 60 * $::AXICLK_PERIOD / 100.0 }] $my_ports 
} else {
  puts {-W- skipping input delay 60% of AXICLK on *}
}

set my_ports [remove_from_collection [get_ports -quiet { * } -filter "direction=~*out"] $::my_out_clock_ports]
set my_clock [get_clocks -quiet { AXICLK }]
if { [sizeof_collection $my_clock]>0 && [sizeof_collection $my_ports]>0 } {
  puts {-I- adding additional output delay of 60% of AXICLK on *}
  set_output_delay -add_delay -max -clock $my_clock  [expr { 60 * $::AXICLK_PERIOD / 100.0 }] $my_ports
} else {
  puts {-W- skipping output delay 60% of AXICLK on *}
}

set my_ports [remove_from_collection [get_ports -quiet { * } -filter "direction=~in*"] $::my_in_clock_ports]
set my_clock [get_clocks -quiet { AICLK }]
if { [sizeof_collection $my_clock]>0 && [sizeof_collection $my_ports]>0 } {
  puts {-I- adding additional input delay of 60% of AICLK on *}
  set_input_delay -add_delay -max -clock $my_clock  [expr { 60 * $::AICLK_PERIOD / 100.0 }] $my_ports 
} else {
  puts {-W- skipping input delay 60% of AICLK on *}
}

set my_ports [remove_from_collection [get_ports -quiet { * } -filter "direction=~*out"] $::my_out_clock_ports]
set my_clock [get_clocks -quiet { AICLK }]
if { [sizeof_collection $my_clock]>0 && [sizeof_collection $my_ports]>0 } {
  puts {-I- adding additional output delay of 60% of AICLK on *}
  set_output_delay -add_delay -max -clock $my_clock  [expr { 60 * $::AICLK_PERIOD / 100.0 }] $my_ports
} else {
  puts {-W- skipping output delay 60% of AICLK on *}
}

set my_ports [remove_from_collection [get_ports -quiet { * } -filter "direction=~in*"] $::my_in_clock_ports]
set my_clock [get_clocks -quiet { OVLCLK }]
if { [sizeof_collection $my_clock]>0 && [sizeof_collection $my_ports]>0 } {
  puts {-I- adding additional input delay of 60% of OVLCLK on *}
  set_input_delay -add_delay -max -clock $my_clock  [expr { 60 * $::OVLCLK_PERIOD / 100.0 }] $my_ports 
} else {
  puts {-W- skipping input delay 60% of OVLCLK on *}
}

set my_ports [remove_from_collection [get_ports -quiet { * } -filter "direction=~*out"] $::my_out_clock_ports]
set my_clock [get_clocks -quiet { OVLCLK }]
if { [sizeof_collection $my_clock]>0 && [sizeof_collection $my_ports]>0 } {
  puts {-I- adding additional output delay of 60% of OVLCLK on *}
  set_output_delay -add_delay -max -clock $my_clock  [expr { 60 * $::OVLCLK_PERIOD / 100.0 }] $my_ports
} else {
  puts {-W- skipping output delay 60% of OVLCLK on *}
}
