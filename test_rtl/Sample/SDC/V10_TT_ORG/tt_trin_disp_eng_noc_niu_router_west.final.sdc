...
global SDC_DIR
set ::DESIGN_NAME tt_trin_disp_eng_noc_niu_router_west
if {![info exists ::tt_global_is_mapped]} { set ::tt_global_is_mapped 1 }
if {![info exists ::DESIGN_NAME]} { set ::DESIGN_NAME {tt_trin_disp_eng_noc_niu_router_west} }
global DESIGN_NAME
...
create_clock -add -name NOCCLK -period $::NOCCLK_PERIOD
...
set my_clock [get_clocks -quiet { vir_NOCCLK }]
if { [sizeof_collection $my_clock]>0 && [sizeof_collection $my_ports]>0 } {
  puts {-I- setting input delay of 70% of vir_NOCCLK on *}
  set_input_delay -max -clock $my_clock  [expr { 70 * $::vir_NOCCLK_PERIOD / 100.0 }] $my_ports
} else {
  puts {-W- skipping input delay 70% of vir_NOCCLK on *}
}

set my_ports [remove_from_collection [get_ports -quiet { * } -filter "direction=~*out"] $::my_out_clock_ports]
set my_clock [get_clocks -quiet { vir_NOCCLK }]
if { [sizeof_collection $my_clock]>0 && [sizeof_collection $my_ports]>0 } {
  puts {-I- setting output delay of 70% of vir_NOCCLK on *}
  set_output_delay -max -clock $my_clock  [expr { 70 * $::vir_NOCCLK_PERIOD / 100.0 }] $my_ports
} else {
  puts {-W- skipping output delay 70% of vir_NOCCLK on *}
}

set my_ports [remove_from_collection [get_ports -quiet { * } -filter "direction=~in*"] $::my_in_clock_ports]
set my_clock [get_clocks -quiet { vir_OVLCLK }]
if { [sizeof_collection $my_clock]>0 && [sizeof_collection $my_ports]>0 } {
  puts {-I- adding additional input delay of 70% of vir_OVLCLK on *}
  set_input_delay -add_delay -max -clock $my_clock  [expr { 70 * $::vir_OVLCLK_PERIOD / 100.0 }] $my_ports 
} else {
  puts {-W- skipping input delay 70% of vir_OVLCLK on *}
}

set my_ports [remove_from_collection [get_ports -quiet { * } -filter "direction=~*out"] $::my_out_clock_ports]
set my_clock [get_clocks -quiet { vir_OVLCLK }]
if { [sizeof_collection $my_clock]>0 && [sizeof_collection $my_ports]>0 } {
  puts {-I- adding additional output delay of 70% of vir_OVLCLK on *}
  set_output_delay -add_delay -max -clock $my_clock  [expr { 70 * $::vir_OVLCLK_PERIOD / 100.0 }] $my_ports
} else {
  puts {-W- skipping output delay 70% of vir_OVLCLK on *}
}
...
if {$::DESIGN_NAME eq "tt_trin_disp_eng_noc_niu_router_west"} {
  set_max_fanout 32 [current_design]
...
#    Generate collapsed synth reports
  ...
  set_false_path -through [get_pins overlay_noc_sec_conf/o_sec_config*]
...
#set non_shitmux [get_cells -quiet -hier *d0nt_clkmux -filter "full_name=~*_dfx_inst*_dft_inst*_occ_inst*tessent_persistent_cell_clock_out_mux*" ]
...
set isTT false
  ...
  set all_user_cust_pg [get_object_name  [get_path_groups {low_thruput reg2mem mem2reg}]]
  foreach sin_pg $all_user_cust_pg {
