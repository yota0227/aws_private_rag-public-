...
if {![info exists ::DESIGN_NAME]} { set ::DESIGN_NAME {tt_dispatch_engine} }
global DESIGN_NAME
...
set my_clock [get_clocks -quiet { ck_feedthru }]
if { [sizeof_collection $my_clock]>0 && [sizeof_collection $my_ports]>0 } {
  puts {-I- setting output delay of 80% of ck_feedthru on *}
  set_output_delay -max -clock $my_clock  [expr { 80 * $::ck_feedthru_PERIOD / 100.0 }] $my_ports
} else {
  puts {-W- skipping output delay 80% of ck_feedthru on *}
}
...
set cur_instance disp_eng_overlay_noc_wrap/disp_eng_overlay_noc_niu_router/disp_eng_overlay_wrapper/overlay_wrapper/
...
# Static inputs: never toggle 
set_false_path -from [get_ports i_static_is_r180*]
set_false_path -from [get_ports i_phys_x*]
set_false_path -from [get_ports i_phys_y*]