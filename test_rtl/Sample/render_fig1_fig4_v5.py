...
"""Fig 1 V0.9 — EDC nodes shown in all three sections per tile:
     NOC boundary box  → noc_clk nodes (NOC N/E/S/W, NIU, sec_fence)
     Chain box         → aiclk nodes   (T6_MISC, IE, SRCB, SFPU, FPU, L1_CLIENT, L1W2)
     Overlay box       → dm_clk nodes  (WDT, APB bridge)
...
# chain box: aiclk-domain nodes only (NOC/OVL nodes moved to their boundary boxes)
...
# ─── compact node row helper ───────────────────────────────────────────────────
def node_table(d, x0, x1, y_start, nodes):
    """Render a compact node table spanning x0..x1, starting at y_start.
    Returns the y coordinate after the last row."""
    C0 = x0 + 8        # node name (left-align)
    C1 = x0 + 350      # clock     (center)
    C2 = x0 + 420      # fault     (left-align)
    C3 = x1 - 42       # severity  (center)
    # header
...
# ═════════════════════════════════════════════════════════════════════════════
# FIG 1 — V0.9: EDC nodes in NOC row, chain box, and OVL row
# ═════════════════════════════════════════════════════════════════════════════
def draw_fig1():
    W, H = 1520, 3060
    img = Image.new("RGB", (W,H), BG)
    ...
    TITLE_H = 120
    GAP = 30
    # V0.9: NOC_H/OVL_H expanded to hold node tables
    # NOC box: 82 base + 14 sep/hdr + 3×14 rows = 138 → use 140
    # OVL box: 82 base + 14 sep/hdr + 2×14 rows = 124 → use 126
    NOC_H = 140
    OVL_H = 126
    # tile heights sized so chain fits its node table
    ...
    ctxt(d,"Fig 1 — One Column (X=0): Complete EDC Ring — 5 Tiles + U-Turn  [V0.9]",W//2,6,f18,BLUE)
    ...
    # ── NOC boundary row (V0.9: node table below DEMUX-A/MUX-B section) ──────
    def noc_row(y0b, sel_note, yi):
        ...
        # center divider only for the DEMUX/MUX section (top 78px)
        ...
        # node table (full-width, below divider)
        d.line([(NB_X0+4, y0b+78),(NB_X1-4, y0b+78)], fill=LGRAY, width=1)
        node_table(d, NB_X0, NB_X1, y0b+80, NOC_NODES)

    # ── Overlay boundary row (V0.9: node table below MUX-A/DEMUX-B section) ──
    def ovl_row(y0b, sel_note, yi):
        ...
        d.line([(NB_X0+4,y0b+20),(NB_X1-4,y0b+20)], fill=LGRAY, width=1)
        # center divider only for the MUX/DEMUX section
        d.line([(TILE_MID, y0b+22),(TILE_MID, y0b+78)], fill=LGRAY, width=1)
        MA = (NB_X0+TILE_MID)//2
        DB = (TILE_MID+NB_X1)//2
    ...
    # ── shared sub-chain box (aiclk nodes only) ────────────────────────────────
    def chain_box(y0b, y1b, chain_label, chain_note, nodes=None):
        ...
        # direction arrows (top portion only, above the table)
        ACX = NB_X0+14
    ...
    bx(d,0,0,W,H,fill=None,outline=BLUE,lw=2)
    save(img, "01_edc_ring_full_architecture_v0.9.png")
