...
**Version:** 0.3
**Date:** 2026-03-18
**N1B0 update:** 2026-03-19 — L1=768 KB/tile (trin config: 16 banks × 3072×128b), per-column clocks (i_ai_clk/i_dm_clk arrays), DFX pass-through wrappers documented.
...
- A **large banked L1 SRAM** (≈1.5 MB per tile) that holds activations, weights, and intermediate tensors. (Baseline 20260221 RTL uses Mimir L1 config: ~1.0 MB per tile. **N1B0 uses `trin` L1 config: 768 KB per tile** — 16 banks × 3072×128b = 48 KB/bank.)
...
> **N1B0 DFX:** tt_tensix_with_l1 includes DFX pass-through wrappers: tt_noc_niu_router_dfx (5 clocks), tt_overlay_wrapper_dfx (5 clocks), tt_instrn_engine_wrapper_dfx (1 clock), tt_t6_l1_partition_dfx (3 clocks). All are wire-assign pass-throughs in N1B0 (INCLUDE_TENSIX_NEO_IJTAG_NETWORK not defined).
...
> **N1B0:** `i_ai_clk` is a per-column array `[SizeX-1:0]` (4 independent AI clocks, one per column). `i_dm_clk` is similarly per-column.
...
| `N1B0 L1 bank depth` | 3072 rows | `tt_t6_l1` | N1B0 trin config: 48 KB/bank, 768 KB/tile (vs baseline Mimir config) |
