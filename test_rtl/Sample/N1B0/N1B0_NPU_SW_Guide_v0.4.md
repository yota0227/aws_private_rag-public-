# Trinity N1B0 — NPU SW Engineer's Complete Guide v0.4
...
Base address: `0x0300_0200`

L1 SRAM port control and hash function configuration.

#### GROUP_HASH_FN_CTRL (offset 0x00)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| ctrl | [31:0] | RW | 0x0 | Hash function control for L1 bank selection | 32-bit hash function control register for L1 bank group selection |

#### GROUP_HASH_FN0_MASK (offset 0x04)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| mask | [31:0] | RW | 0x0 | Hash function 0 mask bits | 32-bit mask applied to address for hash function 0 bank selection |

#### GROUP_HASH_FN1_MASK (offset 0x08)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| mask | [31:0] | RW | 0x0 | Hash function 1 mask bits | 32-bit mask applied to address for hash function 1 bank selection |

#### GROUP_HASH_FN_SEL (offset 0x0C)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| sel | [1:0] | RW | 0x0 | Hash function selection | 0: linear (address bits) \| 1: hash function 0 \| 2: hash function 1 \| 3: reserved |

#### IN_ORDER_MASK (offset 0x10)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| mask | [31:0] | RW | 0x0 | Per-port in-order access mask | 32-bit per-port bitmask; bit N=1 forces in-order (non-pipelined) access on port N |

#### PORT_CTRL_0 through PORT_CTRL_7 (offsets 0x40, 0x80, 0xC0, 0x100, 0x140, 0x180, 0x1C0, 0x200)

One control register per L1 port (ports 0–7).

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| ctrl | [31:0] | RW | 0x0 | L1 port N control (enable, priority, arbitration) | 32-bit L1 port control (enable, priority, arbitration weights); see L1 CSR spec |

#### PORT_STATUS_0 through PORT_STATUS_7 (offsets 0x80, 0xC0, 0x100, 0x140, 0x180, 0x1C0, 0x200, 0x240)

One status register per L1 port (ports 0–7). All fields are RO/W1C (write 1 to clear the sticky error bit).

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| FP_ERROR | [0] | RO/W1C | 0x0 | Floating-point error on port N | Write 0: no effect \| Write 1: clears this bit |
| FP_NAN | [1] | RO/W1C | 0x0 | NaN detected on port N | Write 0: no effect \| Write 1: clears this bit |
| FP_OVERFLOW | [2] | RO/W1C | 0x0 | FP overflow on port N | Write 0: no effect \| Write 1: clears this bit |
| FP_UNDERFLOW | [3] | RO/W1C | 0x0 | FP underflow on port N | Write 0: no effect \| Write 1: clears this bit |
| INT_OVERFLOW | [4] | RO/W1C | 0x0 | Integer overflow on port N | Write 0: no effect \| Write 1: clears this bit |
| FIFO_PARITY_ERR | [5] | RO/W1C | 0x0 | FIFO parity error on port N | Write 0: no effect \| Write 1: clears this bit |
...
Base address: `0x0300_4000` (confirmed from RTL: tt_rocc_accel_reg.svh)

iDMA engine APB status and completion-tracking registers. Note: the CMD_BUF_R/W register map (at offsets within 0x03004000) is covered in §7.5.2. This section covers the per-TID tracking and clock-gate registers.

#### IDMA_STATUS (offset 0x00)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| in_flight | [31:0] | RO | 0x0 | Bit N=1 if TID N has in-flight transfer | Bitmask: bit N=1 means TID N has an in-flight DMA transfer |

#### IDMA_VC_SPACE_BE0 (offset 0x04)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| vc_slots | [31:0] | RO | 0x0 | Available VC slots backend 0 | 32-bit available virtual channel slot count for backend 0 |

#### IDMA_VC_SPACE_BE1 (offset 0x08)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| vc_slots | [31:0] | RO | 0x0 | Available VC slots backend 1 | 32-bit available virtual channel slot count for backend 1 |

#### IDMA_TR_COUNT_0 through IDMA_TR_COUNT_31 (offsets 0x10–0x8C, stride 0x4)

Per-TID completion count. IDMA_TR_COUNT_N at offset `0x10 + N*4`.

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| count | [31:0] | RO | 0x0 | Per-TID N completion count | 32-bit per-TID transfer completion count; read-only; clears via IDMA_CLR |

#### IDMA_THRESHOLD_0 through IDMA_THRESHOLD_31 (offsets 0x90–0x10C, stride 0x4)

Per-TID IRQ threshold. IDMA_THRESHOLD_N at offset `0x90 + N*4`.

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| threshold | [31:0] | RW | 0x0 | Per-TID N IRQ threshold | 32-bit IRQ threshold; IRQ fires when per-TID count reaches this value |

#### IDMA_CLR (offset 0x110)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| clr_mask | [31:0] | W1C | 0x0 | Write bit N to clear TID N completion count and IRQ | Bitmask: write 1 to bit N to clear TID N completion count and deassert IRQ |

#### IDMA_CLK_EN (offset 0x114)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| l1_clk_gate_en | [0] | RW | 0 | L1 flex-client clock gate enable | 0: clock always-on \| 1: clock gated when idle |

#### IDMA_CLK_HYST (offset 0x118)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| hyst | [6:0] | RW | 0x0 | Clock gate hysteresis cycles | 7-bit value N: clock stays on for 2^N cycles after last activity before gating |
...
*Document: Trinity N1B0 — NPU Software Engineer's Guide v0.4*
*Date: 2026-03-25*
*§18 source: N1B0_NPU_sfr_v0.3.csv (3168 rows, 8 subsystems)*
