...
**Date:** 2026-03-16
**Changes from V0.1:**
- Corrected Tensix sub-core ring order: Gtile[0] → instrn_engine_wrapper → Gtile[1] (RTL-verified from `tt_tensix.sv` L222/L225)
- Corrected Gtile internal ring order: general_error(inst+0) → src_parity(inst+1) → dest_parity(inst+2) (verified from `tt_fpu_gtile.sv` L931–L1099)
- Applied corrected Gtile inst encoding: Gtile[0] base=0x20, Gtile[1] base=0x23 (from `tt_tensix_edc_pkg.sv`)
- Corrected NoC2AXI ring order: sec_fence moved to last position before ring exit (verified from `tt_noc2axi.sv` L4851/L4863)
- Corrected NoC2AXI AXI buffer order: mst_rd_ep → mst_rd_routing → mst_wr_ep → mst_wr_routing → mst_wr[0..N] → mst_rd[0..N] → slv_rd (verified from `tt_noc2axi.sv` L3641–L4481)
- Confirmed Dispatch West ring order (no changes needed, verified from `tt_trin_disp_eng_noc_niu_router_west.sv`)

---

> **N1B0 Adaptation Note:** The NOC2AXI segment descriptions below apply to baseline corner tiles (NE_OPT at X=0, NW_OPT at X=3). For N1B0 middle columns (X=1,2):
> - `trinity_noc2axi_router_ne_opt` (X=1) contains: NOC2AXI segment (Y=4, EP=9) + Router segment (Y=3, EP=8) internally
> - `trinity_noc2axi_router_nw_opt` (X=2) contains: NOC2AXI segment (Y=4, EP=14) + Router segment (Y=3, EP=13) internally
> - Ring enters composite module at Y=4 and exits at Y=3 (single external EDC interface per composite)
> - `gen_router` blocks at (X=1,Y=3) and (X=2,Y=3) are EMPTY — no standalone EDC nodes there
...
> **N1B0 Note:** This section applies to baseline architecture only. In N1B0, standalone `NOC2AXI_N_OPT` tiles do not exist at middle columns (X=1,2). Those positions use composite `trinity_noc2axi_router_ne/nw_opt` modules — see N1B0 Adaptation Note at top of this document.
