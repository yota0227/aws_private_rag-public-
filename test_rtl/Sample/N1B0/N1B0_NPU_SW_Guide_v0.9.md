# Trinity N1B0 — NPU SW Engineer's Complete Guide v0.9
...
| v0.4    | 2026-03-25 | SW Team    | §18 Complete SFR Reference added: per-subsystem register field tables with full value_description (all 8 subsystems, from N1B0_NPU_sfr_v0.3.csv) |
| v0.9    | 2026-03-26 | SW Team    | §18 SFR Reference updated from N1B0_NPU_sfr_v0.5.csv: T6_L1_CSR corrected (17 wrong names removed, 87 RTL regs added including RD/RW/WR port ctrl/status and hash fn detail regs); IDMA_APB expanded to full RTL (8 CPUs × 5 blocks, CPU0–CPU7, 0x03004000–0x0300_9FFF); §19 SMN added (Security Monitor, 9 sub-blocks, base 0x03010000); §20 Debug Module added (RISC-V debug v0.13.2, base 0x0300A000); §21 AWM added (Adaptive Workload Manager, base 0x03020000); §2 memory map updated with all new address ranges; §18.2/§18.4 tables updated; §18.9–§18.11 new subsections |
...
- §18 Complete SFR Reference
  - §18.1 CLUSTER_CTRL
  - §18.2 T6_L1_CSR
  - §18.3 LLK_TILE_COUNTERS
  - §18.4 IDMA_APB
  - §18.5 ATT
  - §18.6 CACHE_CTRL
  - §18.7 THCON
  ...
  - §18.10 DEBUG_MODULE
  - §18.11 AWM
- §19 SMN — Security Monitor Network
- §20 Debug Module
- §21 AWM — Adaptive Workload Manager
...
0x0300_0000 – 0x03FF_FFFF   ~16 MB   OVERLAY registers
  0x0300_0000               0x1E4    CLUSTER_CTRL (reset vectors, scratch, clock gating, ECC)
  0x0300_0200               0x16C    T6_L1_CSR (L1 port control, hash functions, in-order)
  0x0300_3000               0x1000   LLK tile counters (4 interfaces × 16 counters)
  0x0300_4000               0x5D78   ROCC accel registers (CPU0–CPU7, CMD_BUF/ADDR_GEN/SIMPLE)
  0x0300_A000               0x104    Debug Module (RISC-V debug v0.13.2 APB)
  0x0301_0000               0x9050   SMN (Security Monitor Network, 9 sub-blocks)
  0x0302_0000               0xB0C    AWM (Adaptive Workload Manager, clock/freq/droop)
...
This section provides complete per-subsystem SFR register field tables derived from
`N1B0_NPU_sfr_v0.3.csv`. Each subsection covers one subsystem with base address,
per-register field tables, and full value_description for every field.

Column key:
- **Bit Field**: field name within the register
- **Bit Range**: bit positions (inclusive)
- **Access**: RO=read-only, RW=read-write, WO=write-only, W1C=write-1-to-clear, RO/W1C=read with sticky clear
- **Reset**: reset value
- **Description**: brief functional description
- **Value Description**: encoding of individual values or field-level semantics
...
Cluster controller: hart reset vectors, scratch registers, clock gating, ECC, bus error, debug, L1 region protection.

#### RESET_VECTOR_0 through RESET_VECTOR_7 (offsets 0x000–0x038, stride 0x8)

One 64-bit register per hart (RESET_VECTOR_N at offset N×8).

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| next_pc | [57:0] | RW | 0x0 | Hart N boot PC (word address) | [57:0] word address (byte addr = value << 3); set before releasing hart from reset |

#### SCRATCH_0 through SCRATCH_31 (offsets 0x040–0x0BC, stride 0x4)

32 general-purpose 32-bit scratch registers for SW use.

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| scratch | [31:0] | RW | 0x0 | General-purpose SW scratch | 32-bit general-purpose SW-use field; no hardware interpretation |

#### ROCC_MEM_CHICKEN_BITS (offset 0x0C0)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| chicken | [31:0] | RW | 0x0 | ROCC memory debug chicken bits | 32-bit debug/workaround chicken bits; individual bits enable specific RTL workarounds |

#### SCATTER_LIST_MAGIC_NUM_LO (offset 0x0C4)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| magic | [31:0] | RW | 0x0 | Scatter-list magic number bits [31:0] | Must match expected magic value (see firmware header) to validate scatter-list |

#### SCATTER_LIST_MAGIC_NUM_HI (offset 0x0C8)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| magic | [31:0] | RW | 0x0 | Scatter-list magic number bits [63:32] | Must match expected magic value (see firmware header) to validate scatter-list |

#### CLOCK_GATING (offset 0x0CC)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| ROCC | [0] | RW | 0 | Enable clock gating for ROCC accelerator | 0: clock always-on \| 1: clock gated when idle |
| IDMA | [1] | RW | 0 | Enable clock gating for iDMA engine | 0: clock always-on \| 1: clock gated when idle |
| CLUSTER_CTRL | [2] | RW | 0 | Enable clock gating for cluster controller | 0: clock always-on \| 1: clock gated when idle |
| CONTEXT_SWITCH | [3] | RW | 0 | Enable clock gating for context switch unit | 0: clock always-on \| 1: clock gated when idle |
| LLK_INTF | [4] | RW | 0 | Enable clock gating for LLK interface | 0: clock always-on \| 1: clock gated when idle |
| SNOOP | [5] | RW | 0 | Enable clock gating for NoC snoop path | 0: clock always-on \| 1: clock gated when idle |
| rsvd | [6] | RO | 0 | Reserved bit | Reserved; write 0; read returns 0 |
| L1_FLEX_CLIENT_IDMA | [7] | RW | 0 | Enable clock gating for L1 flex-client iDMA port | 0: clock always-on \| 1: clock gated when idle |
| L1_FLEX_CLIENT_OVERLAY | [8] | RW | 0 | Enable clock gating for L1 flex-client overlay port | 0: clock always-on \| 1: clock gated when idle |

#### CLOCK_GATING_HYST (offset 0x0D0)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| hyst | [6:0] | RW | 0x0 | Clock gate hysteresis count | 7-bit value N: clock stays on for 2^N cycles after last activity before gating |

#### WB_PC_REG_C0 through WB_PC_REG_C7 (offsets 0x0D8–0x110, stride 0x8)

One 64-bit read-only register per hart capturing the last writeback PC for crash analysis.

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| pc | [57:0] | RO | 0x0 | Hart N writeback PC (crash analysis) | [57:0] word address (byte addr = value << 3) |

#### WB_PC_CTRL (offset 0x118)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| enable_mask | [7:0] | RW | 0x0 | WB PC capture enable, one bit per hart | Bitmask: bit N = hart N; set bit to enable WB PC capture for that hart |

#### ECC_PARITY_CONTROL (offset 0x11C)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| mode | [1:0] | RW | 0x0 | ECC/parity control mode | 0: off (no ECC) \| 1: detect only \| 2: detect+correct \| 3: reserved |

#### ECC_PARITY_STATUS (offset 0x120)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| status | [31:0] | RO/W1C | 0x0 | ECC/parity error status; write-1-to-clear | Write 0: no effect \| Write 1: clears this bit |

#### NOC_SNOOP_TL_MASTER_CFG (offset 0x124)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| cfg | [31:0] | RW | 0x0 | NoC snoop TileLink master enable/configuration | 32-bit NoC snoop TileLink master enable/configuration bitmask |

#### ASSERTS (offset 0x128)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| enable | [31:0] | RW | 0x0 | Assertion enable mask (debug/simulation use) | 32-bit assertion enable bitmask; bit N=1 enables assertion N (simulation/debug) |

#### PREFETCHER_CONTROL (offset 0x12C)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| enable | [0] | RW | 0 | I-cache prefetch enable | 0: disabled \| 1: enabled |
| depth | [3:1] | RW | 0 | Prefetch depth in cache lines | 3-bit prefetch depth in cache lines (0=1 line, max=8 lines) |

#### BUS_ERROR_UNIT_DATA_C0 through BUS_ERROR_UNIT_DATA_C7 (offsets 0x130–0x168, stride 0x8)

One register per hart capturing the bus error address.

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| err_addr | [31:0] | RO/W1C | 0x0 | Bus error address for hart N | 32-bit address that triggered the bus error; write 1 to clear (W1C) |

#### L2_DIR_ERRORS_0 through L2_DIR_ERRORS_3 (offsets 0x170–0x17C, stride 0x4)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| errors | [31:0] | RO/W1C | 0x0 | L2 directory error bank group N | 32-bit error bitmask; each bit=1 indicates an error in the corresponding bank/way; write 1 to clear |

#### L2_BANKS_ERRORS_0 through L2_BANKS_ERRORS_15 (offsets 0x180–0x1BC, stride 0x4)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| errors | [31:0] | RO/W1C | 0x0 | L2 bank N error status | 32-bit error bitmask; each bit=1 indicates an error in the corresponding bank/way; write 1 to clear |

#### DEBUG_DMACTIVE (offset 0x1C0)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| dmactive | [7:0] | RW | 0x0 | RISC-V DM active hart mask (1 bit per hart) | Bitmask: bit N = hart N; set bit to enable RISC-V Debug Module for that hart |

#### DEBUG_DMACTIVEACK (offset 0x1C4)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| ack | [7:0] | RO | 0x0 | Debug module active acknowledge | Bitmask: bit N = hart N; indicates Debug Module is active for that hart |

#### DEBUG_SNOOP (offset 0x1C8)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| cfg | [31:0] | RW | 0x0 | Debug snoop bus configuration | 32-bit NoC snoop TileLink master enable/configuration bitmask |

#### OVERLAY_INFO (offset 0x1CC)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| version | [15:0] | RO | chip-id | Overlay version ID | 16-bit overlay version ID; read-only, set at chip fabrication |
| variant | [31:16] | RO | chip-id | Overlay variant (Tensix/Dispatch/etc.) | 16-bit overlay variant ID (Tensix/Dispatch/NIU); read-only, set at chip fabrication |

#### SW_RAS (offset 0x1D0)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| ras | [31:0] | RW | 0x0 | Software RAS (reliability) register | 32-bit software RAS (reliability/availability/serviceability) error injection register |

#### SBUS_RSINK_RESET_FALLBACK (offset 0x1D4)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| enable | [0] | RW | 0 | SBUS reset sink fallback enable | 0: disabled \| 1: enabled |

#### OVERLAY_CHICKEN_BITS (offset 0x1D8)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| chicken | [31:0] | RW | 0x0 | Debug/workaround chicken bits | 32-bit debug/workaround chicken bits; individual bits enable specific RTL workarounds |

#### L1_ACCESSIBLE_REGION (offset 0x1DC)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| base_mask | [20:0] | RW | 0x0 | L1 accessible region base and mask | 21-bit combined base address and mask for L1 accessible region enforcement |

#### L1_REGION_ERROR (offset 0x1E0)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| err_addr | [20:0] | RO/W1C | 0x0 | L1 region access violation address | 21-bit address of L1 region access violation; write 1 to clear (W1C) |

---

### §18.2 T6_L1_CSR

**Base address:** `0x03000200`  
**RTL source:** `tt_cluster_ctrl_t6_l1_csr_reg.svh`  
**Purpose:** L1 SRAM port control, bank hash functions, in-order access masking.

#### Hash Function Registers (base + 0x000–0x0B8)

| Register | Offset | Abs Addr | Access | Description |
|---|---|---|---|---|
| GROUP_HASH_FN_CTRL | 0x00 | 0x3000200 | RW | Hash function selection: bits[1:0]: 0=linear, 1=fn0, 2=fn1 |
| GROUP_HASH_FN0_MASK | 0x04 | 0x3000204 | RW | Hash fn0: address bits mask (which bits feed the hash) |
| GROUP_HASH_FN0_MATCH | 0x08 | 0x3000208 | RW | Hash fn0: address match value |
| GROUP_HASH_FN0_ADDR4..11 | 0x0C–0x28 | 0x300020C–0x3000228 | RW | Hash fn0: address bit mapping for hash lanes 4–11 |
| GROUP_HASH_FN0_SWAP_ADDR10..21 | 0x2C–0x58 | 0x300022C–0x3000258 | RW | Hash fn0: swap address mapping for bits 10–21 |
| GROUP_HASH_FN1_MASK | 0x5C | 0x300025C | RW | Hash fn1: address bits mask |
| GROUP_HASH_FN1_MATCH | 0x60 | 0x3000260 | RW | Hash fn1: address match value |
| GROUP_HASH_FN1_ADDR4..11 | 0x64–0x80 | 0x3000264–0x3000280 | RW | Hash fn1: address bit mapping lanes 4–11 |
| GROUP_HASH_FN1_SWAP_ADDR10..21 | 0x84–0xB0 | 0x3000284–0x30002B0 | RW | Hash fn1: swap address mapping bits 10–21 |
| IN_ORDER_MASK | 0xB4 | 0x30002B4 | RW | Bit N=1: port N requires in-order access |
| IN_ORDER_MATCH | 0xB8 | 0x30002B8 | RW | In-order match value (paired with mask) |

#### Port Control/Status Registers (base + 0xBC–0x168)

Three port types: RW (read-write), RD (read-only), WR (write-only).

| Register Group | Offsets | Abs Addr Range | Access | Count | Description |
|---|---|---|---|---|---|
| RW_PORT_CTRL_0..5 | 0xBC–0xD0 | 0x30002BC–0x30002D0 | RW | 6 | Read-write port N control: bit[0]=enable, bit[1]=stall-on-error |
| RW_PORT_STATUS_0..5 | 0xD4–0xE8 | 0x30002D4–0x30002E8 | RO | 6 | RW port N status: bit[0]=busy, bit[1]=error; W1C on error |
| RD_PORT_CTRL_0..7 | 0xEC–0x108 | 0x30002EC–0x3000308 | RW | 8 | Read-only port N control |
| RD_PORT_STATUS_0..7 | 0x10C–0x128 | 0x300030C–0x3000328 | RO | 8 | Read-only port N status |
| WR_PORT_CTRL_0..7 | 0x12C–0x148 | 0x300032C–0x3000348 | RW | 8 | Write-only port N control |
| WR_PORT_STATUS_0..7 | 0x14C–0x168 | 0x300034C–0x3000368 | RO | 8 | Write-only port N status |

**Port CTRL bits:** `[0]` enable port (1=enabled) | `[1]` stall on parity error (1=stall)  
**Port STATUS bits:** `[0]` port busy | `[1]` parity error (W1C) | `[2]` overflow (W1C) | `[3]` underflow (W1C)

```c
// Example: enable all RW ports
#define T6_L1_CSR_BASE   0x03000200UL
#define REG32(b,o)       (*(volatile uint32_t*)((b)+(o)))

// Enable RW port 0 (offset 0xBC)
REG32(T6_L1_CSR_BASE, 0xBC) = 0x1;

// Check and clear RW port 0 parity error (offset 0xD4)
uint32_t st = REG32(T6_L1_CSR_BASE, 0xD4);
if (st & 0x2) {
    REG32(T6_L1_CSR_BASE, 0xD4) = 0x2;  // W1C clear
}
```
...
Base address: `0x0300_3000`

4 interfaces × 16 counters per interface. Address formula:
`base + interface_index × 0x400 + counter_index × 0x40`

The register names in the CSV follow the pattern `INTF{I}_CNT{N}_{TYPE}` where I=0–3, N=0–15.

#### Per-Counter Register Structure (9 register types, same for every counter)

The following table applies to each counter at its base address (interface I, counter N) = `0x03003000 + I*0x400 + N*0x40`:

| Register Type | Offset | Bit Field | Bit Range | Access | Reset | Description | Value Description |
|---------------|--------|-----------|-----------|--------|-------|-------------|-------------------|
| RESET | +0x04 | reset_val | [0] | WO | 0x0 | Write 1 to reset counter to 0 | 0: normal \| 1: reset asserted |
| POSTED | +0x08 | count | [31:0] | RW | 0x0 | Posted tile count (producer increments) | 32-bit producer tile counter; incremented by firmware/HW on each tile produced |
| ACKED | +0x0C | count | [31:0] | RW | 0x0 | Acked tile count (consumer increments) | 32-bit consumer tile counter; incremented by firmware/HW on each tile consumed |
| BUFFER_CAPACITY | +0x10 | capacity | [31:0] | RW | 0x0 | Maximum buffer capacity in tiles | 32-bit maximum buffer capacity in tiles; set by firmware |
| READ_POSTED | +0x14 | count | [31:0] | RO | 0x0 | Read current posted count | 32-bit producer tile counter; read-only snapshot |
| READ_ACKED | +0x18 | count | [31:0] | RO | 0x0 | Read current acked count | 32-bit consumer tile counter; read-only snapshot |
| ERROR_STATUS | +0x1C | err | [0] | RO/W1C | 0x0 | Counter error/overflow flag | Write 0: no effect \| Write 1: clears this bit |
| TILES_AVAIL_THRESHOLD | +0x38 | thresh | [31:0] | RW | 0x0 | Threshold: IRQ when (posted-acked) >= thresh | 32-bit threshold; IRQ fires when (posted - acked) >= threshold |
| TILES_FREE_THRESHOLD | +0x3C | thresh | [31:0] | RW | 0x0 | Threshold: IRQ when (capacity-(posted-acked)) >= thresh | 32-bit threshold; IRQ fires when (capacity - (posted - acked)) >= threshold |

**Address examples:**
- INTF0_CNT0_POSTED: `0x03003008`
- INTF0_CNT1_POSTED: `0x03003048`
- INTF1_CNT0_POSTED: `0x03003408`
- INTF3_CNT15_TILES_FREE_THRESHOLD: `0x03003FFC`

---

### §18.4 IDMA_APB

**RTL source:** `tt_rocc_accel_reg.svh`  
**Purpose:** iDMA engine command buffers and address generators for 8 RISC-V CPUs.  
**Address range:** `0x03004000` – `0x0300_9FFF`

#### Register File Block Map

Each CPU has 5 register file blocks. All CPUs have identical internal register layouts.

| CPU | CMD_BUF_R | CMD_BUF_W | ADDRESS_GEN_R | ADDRESS_GEN_W | SIMPLE_CMD_BUF |
|-----|-----------|-----------|---------------|---------------|----------------|
| CPU0 | 0x3004000 | 0x3004200 | 0x3004400 | 0x3004600 | 0x3004800 |
| CPU1 | 0x3004C00 | 0x3004E00 | 0x3005000 | 0x3005200 | 0x3005400 |
| CPU2 | 0x3005800 | 0x3005A00 | 0x3005C00 | 0x3005E00 | 0x3006000 |
| CPU3 | 0x3006400 | 0x3006600 | 0x3006800 | 0x3006A00 | 0x3006C00 |
| CPU4 | 0x3007000 | 0x3007200 | 0x3007400 | 0x3007600 | 0x3007800 |
| CPU5 | 0x3007C00 | 0x3007E00 | 0x3008000 | 0x3008200 | 0x3008400 |
| CPU6 | 0x3008800 | 0x3008A00 | 0x3008C00 | 0x3008E00 | 0x3009000 |
| CPU7 | 0x3009400 | 0x3009600 | 0x3009800 | 0x3009A00 | 0x3009C00 |

Block size: CMD_BUF_R/W = 0x178, ADDRESS_GEN_R/W = 0xC8, SIMPLE_CMD_BUF = 0x178

#### CMD_BUF Register Layout (per CPU, offsets from block base)

| Offset | Register | Access | Description |
|--------|----------|--------|-------------|
| 0x000 | IE | RW | Interrupt enable bitmask |
| 0x008 | IP | W1C | Interrupt pending (write 1 to clear) |
| 0x010 | WR_SENT_TR_ID | RO | Last transaction ID sent (write path) |
| 0x018 | TR_ACK_TR_ID | RO | Last transaction ID acknowledged |
| 0x020 | SRC_ADDR | RW | Source byte address |
| 0x028 | SRC_BASE | RW | Source 2D base address |
| 0x030 | SRC_SIZE | RW | Source 2D region size (bytes) |
| 0x038 | SRC_COORD | RW | Source tile XY coordinates |
| 0x040 | DEST_ADDR | RW | Destination byte address |
| 0x048 | DEST_BASE | RW | Destination 2D base address |
| 0x050 | DEST_SIZE | RW | Destination 2D region size (bytes) |
| 0x058 | DEST_COORD | RW | Destination tile XY coordinates |
| 0x060 | LEN_BYTES | RW | Transfer length in bytes |
| 0x068 | REQ_VC | RW | Request virtual channel |
| 0x070 | REQ_VC_BASE | RW | Request VC base (for VC range mode) |
| 0x078 | REQ_VC_SIZE | RW | Request VC range size |
| 0x080 | RESP_VC | RW | Response virtual channel |
| 0x088 | RESP_VC_BASE | RW | Response VC base |
| 0x090 | RESP_VC_SIZE | RW | Response VC range size |
| 0x098 | TR_ID | RW | Transaction ID to assign |
| 0x0A0 | TR_ID_BASE | RW | TR ID base for range allocation |
| 0x0A8 | TR_ID_SIZE | RW | TR ID range size |
| 0x0B0 | MCAST_EXCLUDE | RW | Multicast exclude tile bitmap |
| 0x0B8 | SCATTER_LIST_ADDR | RW | Scatter list memory address |
| 0x0C0 | SCATTER_BASE_ADDR | RW | Scatter 2D base address |
| 0x0C8 | SCATTER_INDEX | RW | Scatter list current index |
| 0x0D0 | SCATTER_TIMES | RW | Number of scatter iterations |
| 0x0D8 | SCATTER_ADDR_0 | RW | Scatter target address 0 |
| 0x0E0 | SCATTER_ADDR_1 | RW | Scatter target address 1 |
| 0x0E8 | SCATTER_ADDR_2 | RW | Scatter target address 2 |
| 0x0F0 | SCATTER_ADDR_3 | RW | Scatter target address 3 |
| 0x0F8 | INLINE_DATA | RW | Inline write data (small transfers) |
| 0x100 | MAX_BYTES_IN_PACKET | RW | Maximum bytes per NoC packet |
| 0x108 | MCAST_DESTS | RW | Multicast destination coordinate bitmap |
| 0x110 | L1_ACCUM_CFG | RW | L1 accumulate mode configuration |
| 0x118 | AXI_OPT_1 | RW | AXI optimization options (burst, cache) |
| 0x120 | AXI_OPT_2 | RW | AXI optimization options 2 |
| 0x128 | AUTOINC | RW | Address auto-increment per transfer |
| 0x130 | PACKET_TAGS | RW | Per-packet NoC tag configuration |
| 0x138 | DEBUG | RO | Debug state register |
| 0x140 | MISC | RW | Miscellaneous configuration |
| 0x148 | PER_TR_ID_IE_0 | RW | Per-TID interrupt enable [31:0] |
| 0x150 | PER_TR_ID_IE_1 | RW | Per-TID interrupt enable [63:32] |
| 0x158 | PER_TR_ID_IE_2 | RW | Per-TID interrupt enable [95:64] |
| 0x160 | PER_TR_ID_IP_0 | W1C | Per-TID interrupt pending [31:0] |
| 0x168 | PER_TR_ID_IP_1 | W1C | Per-TID interrupt pending [63:32] |
| 0x170 | PER_TR_ID_IP_2 | W1C | Per-TID interrupt pending [95:64] |

#### ADDRESS_GEN Register Layout (per CPU, offsets from block base, size 0xC8)

| Offset | Register | Description |
|--------|----------|-------------|
| 0x00 | SRC_BANK_BASE | Source bank base address |
| 0x08 | SRC_BANK_CURRENT | Source bank current pointer |
| 0x10 | SRC_BANK_SIZE | Source bank size |
| 0x18 | SRC_BANK_SKIP | Source bank skip (stride between banks) |
| 0x20 | SRC_FACE_SIZE | Source face size (2D tile face) |
| 0x28 | SRC_INNER_ADDRESS | Source inner loop start address |
| 0x30 | SRC_INNER_END | Source inner loop end address |
| 0x38 | SRC_INNER_STRIDE | Source inner loop stride |
| 0x40 | SRC_OUTER_ADDRESS | Source outer loop address |
| 0x48 | SRC_GEN_ADDRESS | Source generated address (RO) |
| 0x50 | DEST_BANK_BASE | Destination bank base |
| 0x58 | DEST_BANK_CURRENT | Destination bank current pointer |
| 0x60 | DEST_BANK_SIZE | Destination bank size |
| 0x68 | DEST_BANK_SKIP | Destination bank skip |
| 0x70 | DEST_FACE_SIZE | Destination face size |
| 0x78 | DEST_INNER_ADDRESS | Destination inner loop start |
| 0x80 | DEST_INNER_END | Destination inner loop end |
| 0x88 | DEST_INNER_STRIDE | Destination inner loop stride |
| 0x90 | DEST_OUTER_ADDRESS | Destination outer loop address |
| 0x98 | DEST_GEN_ADDRESS | Destination generated address (RO) |
| 0xA0–0xC8 | (additional config) | Extended address generation config |

```c
// Example: CPU0 CMD_BUF_R — 1D DMA from DRAM SI0 to local L1
#define CPU0_CMD_BUF_R   0x03004000UL
#define CMD_REG(off)     (*(volatile uint32_t*)(CPU0_CMD_BUF_R + (off)))

void cpu0_dma_read(uint64_t src_dram, uint32_t dst_l1, uint32_t len) {
    CMD_REG(0x020) = (uint32_t)(src_dram);         // SRC_ADDR lo
    CMD_REG(0x024) = (uint32_t)(src_dram >> 32);   // SRC_ADDR hi
    CMD_REG(0x040) = dst_l1;                       // DEST_ADDR
    CMD_REG(0x060) = len;                          // LEN_BYTES
    CMD_REG(0x068) = 0;                            // REQ_VC = 0 (weight)
    CMD_REG(0x080) = 1;                            // RESP_VC = 1
    CMD_REG(0x098) = 0;                            // TR_ID = 0
    CMD_REG(0x148) = (1u << 0);                    // PER_TR_ID_IE_0: enable TID0 IRQ
    // Trigger: write IE global enable
    CMD_REG(0x000) = 0x1;
}
```
...
Address Translation Table: 16 mask entries, 1024 endpoint entries, 32 dynamic routing entries.
Total size: 0x3000 bytes (12 KB).

#### ATT_MASK_0 through ATT_MASK_15 (offsets 0x000–0x03C, stride 0x4)

16 address mask registers for the ATT address-range matching table. ATT_MASK_N at offset `N*4`.

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| mask | [31:0] | RW | 0x0 | Address mask for ATT entry N | Address mask; bits set = wildcard bits for ATT address range matching |

#### ATT_ENDPOINT_0 through ATT_ENDPOINT_1023 (offsets 0x100–0x2FF8, stride 0x8)

1024 64-bit endpoint entries. ATT_ENDPOINT_N at offset `0x100 + N*8`.

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| endpoint | [63:0] | RW | 0x0 | ATT endpoint entry N: {y_coord[5:0], x_coord[5:0], local_addr} | 64-bit endpoint: {y_coord[5:0], x_coord[5:0], local_addr[51:0]}; target tile = X*5+Y |

#### ATT_DYNAMIC_0 through ATT_DYNAMIC_31 (offsets 0x2000–0x20F8, stride 0x8)

32 64-bit dynamic routing table entries. ATT_DYNAMIC_N at offset `0x2000 + N*8`.

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| routing | [63:0] | RW | 0x0 | Dynamic routing table entry N | 56-bit dynamic routing table entry encoding NoC path for dynamic routing mode |

---

### §18.6 CACHE_CTRL

Base address: `0x0401_0000`

Overlay CPU L1/L2 cache controller registers for configuration readout and cache maintenance operations.

#### CONFIGURATION (offset 0x000)

Read-only hardware configuration register.

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| banks | [7:0] | RO | 0x0 | Number of cache banks | Number of cache banks (read-only hardware configuration) |
| ways | [15:8] | RO | 0x0 | Number of ways per set | Number of cache ways per set (read-only hardware configuration) |
| lgsets | [23:16] | RO | 0x0 | log2(sets per way) | log2(number of sets per way) — read-only hardware configuration |
| lgblockbytes | [31:24] | RO | 0x0 | log2(bytes per cache line) | log2(bytes per cache line) — read-only hardware configuration |

#### FLUSH64 (offset 0x200)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| flush_addr | [63:0] | WO | 0x0 | Write address to flush 64-bit aligned cache line | Write target address to flush (write-back dirty data) the matching cache line |

#### FLUSH32 (offset 0x240)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| flush_addr | [31:0] | WO | 0x0 | Write address to flush 32-bit aligned cache line | Write target address to flush (write-back dirty data) the matching cache line |

#### INVALIDATE64 (offset 0x280)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| addr | [63:0] | WO | 0x0 | Invalidate cache line at 64-bit address | Write address to invalidate (discard without writeback) the matching cache line |

#### INVALIDATE32 (offset 0x2C0)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| addr | [31:0] | WO | 0x0 | Invalidate cache line at 32-bit address | Write address to invalidate (discard without writeback) the matching cache line |

#### FULLINVALIDATE (offset 0x300)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| trigger | [31:0] | WO | 0x0 | Write any value to fully invalidate all cache lines | Write any value to invalidate all cache lines (entire cache) |

#### INVALIDATEFLUSHDIRTY (offset 0x340)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| addr | [63:0] | WO | 0x0 | Invalidate and flush dirty line at address | Write address to flush dirty data then invalidate the matching cache line |

---

### §18.7 THCON

Base address: `0x0180_0000`

Thread Configuration (THCON) registers: unpacker and packer format and mode configuration. Written by TRISC0 (unpacker) and TRISC2 (packer) via `wrcfg` custom instructions.

#### UNPACKER0_REG0 (offset 0x000)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| out_data_format | [3:0] | RW | 0 | Output format code to SRCA | 0:FP32 \| 1:FP16 \| 2:BF16 \| 3:TF32 \| 4:FP8_E4M3 \| 5:FP8_E5M2 \| 6:MX_FP8_E4M3 \| 7:MX_FP8_E5M2 \| 8:MX_FP6_E2M3 \| 9:MX_FP6_E3M2 \| 10:MX_FP4 \| 11:MX_INT8 \| 12:INT32 \| 13:INT16 \| 14:INT8 \| 15:FP16B \| 16:INVALID |
| in_data_format | [7:4] | RW | 0 | Input format code from L1 | 0:FP32 \| 1:FP16 \| 2:BF16 \| 3:TF32 \| 4:FP8_E4M3 \| 5:FP8_E5M2 \| 6:MX_FP8_E4M3 \| 7:MX_FP8_E5M2 \| 8:MX_FP6_E2M3 \| 9:MX_FP6_E3M2 \| 10:MX_FP4 \| 11:MX_INT8 \| 12:INT32 \| 13:INT16 \| 14:INT8 \| 15:FP16B \| 16:INVALID |
| tilize_mode | [15:8] | RW | 0 | Tilize mode | 0: normal tile mode \| 1: row-major tilize mode |
| transpose | [19:16] | RW | 0 | Enable 90-degree transpose | 0: no transpose \| 1: 90-degree transpose enabled |
| enable_arg_fifo | [20] | RW | 0 | Enable streaming argument FIFO | 0: disabled \| 1: enabled |
| bcast_wrap_tile_dim | [31:24] | RW | 0 | Broadcast wrap dimension | 8-bit broadcast wrap tile dimension for SRCA/SRCB broadcast |

#### UNPACKER0_REG1 (offset 0x004)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| y_stride | [15:0] | RW | 0 | Row stride in elements | Row stride in elements for 2D access |
| unpack_stride_row_mask | [31:16] | RW | 0 | Per-row skip mask for strided access | 16-bit per-row skip mask for strided unpack access |

#### UNPACKER0_REG2 (offset 0x008)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| src_z_stride | [31:0] | RW | 0 | Z-plane stride in bytes | Z-plane stride in bytes for 3D tensor access |

#### PACKER0_REG0 (offset 0x010)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| out_data_format | [3:0] | RW | 0 | Output format code to L1 | 0:FP32 \| 1:FP16 \| 2:BF16 \| 3:TF32 \| 4:FP8_E4M3 \| 5:FP8_E5M2 \| 6:MX_FP8_E4M3 \| 7:MX_FP8_E5M2 \| 8:MX_FP6_E2M3 \| 9:MX_FP6_E3M2 \| 10:MX_FP4 \| 11:MX_INT8 \| 12:INT32 \| 13:INT16 \| 14:INT8 \| 15:FP16B \| 16:INVALID |
| in_data_format | [7:4] | RW | 0 | DEST register format code | 0:FP32 \| 1:FP16 \| 2:BF16 \| 3:TF32 \| 4:FP8_E4M3 \| 5:FP8_E5M2 \| 6:MX_FP8_E4M3 \| 7:MX_FP8_E5M2 \| 8:MX_FP6_E2M3 \| 9:MX_FP6_E3M2 \| 10:MX_FP4 \| 11:MX_INT8 \| 12:INT32 \| 13:INT16 \| 14:INT8 \| 15:FP16B \| 16:INVALID |
| read_mode | [9:8] | RW | 0 | Read mode | 0: normal \| 1: transposed \| 2: untilize |
| dest_reg_wrap_mode | [11:10] | RW | 0 | Double-buffer ping/pong mode | 0: single-buffer \| 1: ping-pong double-buffer \| 2-3: reserved |
| l1_acc | [12] | RW | 0 | L1 accumulate instead of overwrite | 0: inactive \| 1: active |
| zero_write | [13] | RW | 0 | Write zeros (INIT/clear mode) | 0: inactive \| 1: active |
| stoch_rnd_en | [14] | RW | 0 | Stochastic rounding enable | 0: disabled \| 1: enabled |
| relu_mode | [17:16] | RW | 0 | ReLU mode | 0: none \| 1: zero negative values \| 2: compare threshold \| 3: clamp to range |
| exp_threshold_en | [18] | RW | 0 | Exponent threshold pruning enable | 0: disabled \| 1: enabled |
| exp_threshold_val | [27:19] | RW | 0 | Exponent value below which outputs are zeroed | 9-bit exponent value; outputs with exponent below this are zeroed |
| int_descale_en | [28] | RW | 0 | INT8 descale right-shift enable | 0: disabled \| 1: enabled |
| int_descale_mode | [30:29] | RW | 0 | Descale right-shift amount selection | 2-bit select for descale shift amount (0-3 maps to shift configurations) |

#### PACKER0_REG1 (offset 0x014)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| dst_addr_offset | [20:0] | RW | 0 | L1 destination byte address offset | 21-bit byte address offset into L1 for packed output |
| src_z_stride | [31:21] | RW | 0 | Packer Z-plane stride | Z-plane stride in bytes for 3D tensor access |

#### PACKER0_REG2 (offset 0x018)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| edge_mask0 | [15:0] | RW | 0xFFFF | First face edge mask (16 element bits) | 16-bit mask; bit N=1 includes element N in this face edge |
| edge_mask1 | [31:16] | RW | 0xFFFF | Second face edge mask | 16-bit mask; bit N=1 includes element N in this face edge |

#### UNPACKER1_REG0 (offset 0x020)

Same field layout as UNPACKER0_REG0 but controls SRCB (activations) instead of SRCA.

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| out_data_format | [3:0] | RW | 0 | Output format code to SRCB | Same format encoding as UNPACKER0_REG0.out_data_format |
| in_data_format | [7:4] | RW | 0 | Input format code from L1 | Same format encoding as UNPACKER0_REG0.in_data_format |
| tilize_mode | [15:8] | RW | 0 | Tilize mode | 0: normal tile mode \| 1: row-major tilize mode |
| transpose | [19:16] | RW | 0 | Enable 90-degree transpose | 0: no transpose \| 1: 90-degree transpose enabled |
| enable_arg_fifo | [20] | RW | 0 | Enable streaming argument FIFO | 0: disabled \| 1: enabled |
| bcast_wrap_tile_dim | [31:24] | RW | 0 | Broadcast wrap dimension | 8-bit broadcast wrap tile dimension for SRCB broadcast |

#### UNPACKER1_REG1 (offset 0x024)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| src_z_stride | [31:0] | RW | 0 | Z-plane stride in bytes | Z-plane stride in bytes for 3D tensor access |

#### UNPACKER1_REG2 (offset 0x028)
...
#### PACKER1_REG0 (offset 0x030)

Same field layout as PACKER0_REG0 but controls the secondary packer (PACKER1).

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| out_data_format | [3:0] | RW | 0 | Output format code to L1 | Same format encoding as PACKER0_REG0.out_data_format |
| in_data_format | [7:4] | RW | 0 | DEST register format code | Same format encoding as PACKER0_REG0.in_data_format |
| read_mode | [9:8] | RW | 0 | Read mode | 0: normal \| 1: transposed \| 2: untilize |
| dest_reg_wrap_mode | [11:10] | RW | 0 | Double-buffer ping/pong mode | 0: single-buffer \| 1: ping-pong double-buffer \| 2-3: reserved |
| l1_acc | [12] | RW | 0 | L1 accumulate instead of overwrite | 0: inactive \| 1: active |
| zero_write | [13] | RW | 0 | Write zeros (INIT/clear mode) | 0: inactive \| 1: active |
...
Base address: `0x0200_0000` (NOC register space)

The NOC subsystem in the SFR CSV contains three sub-regions:

1. **ATT tables (mirrored at NOC base offset):** ATT_MASK_0–15 at base+0x10000, ATT_ENDPOINT_0–1023 at base+0x10100, ATT_DYNAMIC_0–31 at base+0x12000. These are the same physical registers as §18.5 but accessed via the NOC register region rather than the ATT base. See §18.5 for full field descriptions.

2. **NIU and Router Configuration Registers:** Four control/status registers covering both NOC0 and NOC1 instances.

#### NOC0_NIU_CFG (absolute address 0x02020000, offset 0x020000 from NOC base)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| cfg | [31:0] | RW | 0x0 | NIU 0 configuration register | 32-bit configuration register; see NIU/Router CSR documentation for field details |

#### NOC0_NIU_STATUS (absolute address 0x02020010, offset 0x020010)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| status | [31:0] | RO | 0x0 | NIU 0 status register | 32-bit read-only status; see NIU/Router CSR documentation for field details |

#### NOC0_ROUTER_CFG (absolute address 0x02020020, offset 0x020020)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| cfg | [31:0] | RW | 0x0 | Router 0 configuration register | 32-bit configuration register; see NIU/Router CSR documentation for field details |

#### NOC1_NIU_CFG (absolute address 0x02030000, offset 0x030000)

| Bit Field | Bit Range | Access | Reset | Description | Value Description |
|-----------|-----------|--------|-------|-------------|-------------------|
| cfg | [31:0] | RW | 0x0 | NIU 1 configuration register (dual-NOC) | 32-bit configuration register; see NIU/Router CSR documentation for field details |

#### ATT Registers (NOC-region aliases)

The following ATT table entries are accessible at the NOC base offset as well as at the primary ATT base (0x02010000). The register structure and field encoding are identical to §18.5.

| Register Group | NOC Offset | Count | Description |
|----------------|------------|-------|-------------|
| ATT_MASK_0–15 | 0x10000–0x1003C | 16 | Address masks (stride 0x4) |
| ATT_ENDPOINT_0–1023 | 0x10100–0x12FF8 | 1024 | 64-bit endpoint entries (stride 0x8) |
| ATT_DYNAMIC_0–31 | 0x12000–0x120F8 | 32 | 64-bit dynamic routing entries (stride 0x8) |

For full field descriptions of ATT_MASK, ATT_ENDPOINT, and ATT_DYNAMIC, see §18.5.

---

### §18.9 SMN — Security Monitor Network

**Base address:** `0x03010000`  
**RTL source:** `smn_reg.svh`  
**Purpose:** Controls per-tile NoC access permissions, address translation, die-to-die BAR, and inter-tile mailboxes.

#### Sub-block Map

| Sub-block | Base | Regs | Description |
|---|---|---|---|
| MST_COMMON | 0x3010000 | ~10 | Master control, command/response timeouts, IRQ |
| MST_MAIN | 0x3011000 | ~10 | Token configuration, idle timer |
| ADDR_TRANS | 0x3012000 | ~16+ | Address translation mask table entries |
| MST_CMD_BUF | 0x3013000 | ~8 | SMN command buffer (REG0..REG7) |
| SLAVE_MAILBOX_0 | 0x3017000 | 3 | Tile mailbox 0: MBOXW(WO), MBOXR(RO), STATUS |
| SLAVE_MAILBOX_1 | 0x3017100 | 3 | Tile mailbox 1: MBOXW(WO), MBOXR(RO), STATUS |
| SLV_BLOCK | 0x3017800 | ~16 | Slave scratch registers, slave control |
| SLV_NOC_SEC | 0x3018000 | ~10 | NoC security: X/Y coord, endpoint ID, mesh orientation |
| SLV_D2D | 0x3019000 | ~16 | Die-to-die BAR base addresses |

#### Key Registers

| Register | Abs Addr | Access | Description |
|---|---|---|---|
| SMN_MST_CTRL | 0x3010000 | RW | Master enable, error mask, global IRQ enable |
| SMN_MST_CMD_TIMEOUT | 0x3010008 | RW | Command timeout in NoC cycles (0=disabled) |
| SMN_MST_RESP_TIMEOUT | 0x3010010 | RW | Response timeout in NoC cycles |
| SMN_MST_TOKEN_CONFIG | 0x3011000 | RW | Token ring configuration |
| SMN_ADDR_TRANS_MASK_TABLE_CFG_0..N | 0x3012000+ | RW | Address translation mask entry N |
| SLAVE_MAILBOX_0_MBOXW | 0x3017000 | WO | Write to mailbox 0 (triggers interrupt on reader) |
| SLAVE_MAILBOX_0_MBOXR | 0x3017008 | RO | Read mailbox 0 (clears message) |
| SLAVE_MAILBOX_0_STATUS | 0x3017010 | RO | Mailbox 0 status: bit[0]=full, bit[1]=empty |
| SMN_NOC_SEC_X_Y | 0x3018000 | RW | This tile's X/Y coordinates for NoC security |
| SMN_NOC_SEC_MESH_X_Y_ORIENTATION | 0x3018008 | RW | Mesh orientation register |
| SMN_NOC_SEC_ENDPOINT_ID | 0x3018010 | RW | Endpoint ID for security checks |
| SMN_SLV_D2D_BAR_BASE_0 | 0x3019000 | RW | D2D BAR 0 base address |
| SMN_SLV_D2D_BAR_BASE_1 | 0x3019008 | RW | D2D BAR 1 base address |

```c
// Example: read tile mailbox 0
#define SMN_MBOX0_R  0x03017008UL

uint32_t smn_mbox_read(void) {
    // Check full before reading
    uint32_t st = *(volatile uint32_t*)0x03017010UL;
    if (st & 0x1) {  // bit[0]=full
        return *(volatile uint32_t*)SMN_MBOX0_R;
    }
...
**RTL source:** `tt_debug_module_apb_reg.svh`  
**Standard:** RISC-V External Debug Support v0.13.2  
**Purpose:** RISC-V debug module — halt/resume harts, abstract commands, program buffer, system bus access.

#### Register Map

| Register | Offset | Abs Addr | Access | Description |
|---|---|---|---|---|
| DATA | 0x010 | 0x300A010 | RW | Abstract command data registers |
| DMCONTROL | 0x040 | 0x300A040 | RW | Debug module control: `dmactive[0]`, `ndmreset[1]`, `hartselhi[15:6]`, `hartsello[25:16]`, `resumereq[30]`, `haltreq[31]` |
| STATUS | 0x044 | 0x300A044 | RO | `anyhalted[9]`, `allhalted[8]`, `anyrunning[11]`, `allrunning[10]`, `version[3:0]` |
| DMI_HARTINFO | 0x048 | 0x300A048 | RO | `dataaddr[11:0]`, `datasize[15:12]`, `nscratch[23:20]` |
| HALTSUMMARY1 | 0x04C | 0x300A04C | RO | Halt summary for harts 32–63 |
| HAWINDOW | 0x054 | 0x300A054 | RW | Hart array window select |
| ABSTRACTS | 0x058 | 0x300A058 | RO | Abstract command status: `cmderr[10:8]`, `busy[12]`, `progbufsize[28:24]`, `datacount[3:0]` |
| COMMAND | 0x05C | 0x300A05C | WO | Abstract command: `cmdtype[31:24]`, `control[23:0]` |
| ABSTRACTAUTO | 0x060 | 0x300A060 | RW | Automatic abstract command trigger |
| PROGBUF | 0x080 | 0x300A080 | RW | Program buffer (up to 16 words of RISC-V instructions) |
| STATUS2 | 0x0C8 | 0x300A0C8 | RO | Extended status register |
| SBCS | 0x0E0 | 0x300A0E0 | RW | System bus access control/status: `sbaccess[19:17]`, `sbautoincrement[16]`, `sbreadondata[15]`, `sbbusy[21]`, `sberror[14:12]` |
| SBADDR0 | 0x0E4 | 0x300A0E4 | RW | System bus address bits [31:0] |
| SBADDR1 | 0x0E8 | 0x300A0E8 | RW | System bus address bits [63:32] |
| SBDATA0 | 0x0F0 | 0x300A0F0 | RW | System bus data bits [31:0] |
| SBDATA1 | 0x0F4 | 0x300A0F4 | RW | System bus data bits [63:32] |
| HALTSUMMARY0 | 0x100 | 0x300A100 | RO | Halt summary for harts 0–31 (bit N=1 means hart N halted) |

```c
// Example: halt hart 0 via debug module
#define DBG_BASE   0x0300A000UL
#define DBG_REG(o) (*(volatile uint32_t*)(DBG_BASE+(o)))

void debug_halt_hart0(void) {
    // Write DMCONTROL: dmactive=1, haltreq=1, hartsel=0
    DBG_REG(0x040) = (1u<<31) | (1u<<0);  // haltreq + dmactive
    // Poll STATUS.anyhalted
    while (!(DBG_REG(0x044) & (1u<<9))) {}
    // Clear haltreq
    DBG_REG(0x040) = (1u<<0);  // dmactive only
}
```

### §18.11 AWM — Adaptive Workload Manager

**Base address:** `0x03020000`  
**RTL source:** `tt_neo_awm_wrap_reg.svh`  
**Purpose:** Dynamic clock frequency management, clock gate managers, droop detectors, temperature sensor, PLL/PVT monitoring.

#### Sub-block Map

| Sub-block | Base | Regs | Description |
|---|---|---|---|
| AWM_GLOBAL | 0x3020000 | ~12 | Global resource config, dynamic scheme selection |
| AWM_FREQUENCY0..5 | 0x3020100–0x3020240 | 6×~8 | Frequency control words (FCW integer/fractional parts) per clock domain |
| AWM_CGM0..2 | 0x3020280–0x3020480 | 3×~16 | Clock gate managers: enables, open-loop, threshold |
| DROOP_0..3 | 0x3020500–0x3020800 | 4×~16 | Droop detectors per voltage domain |
| TEMP_SENSOR | 0x3020900 | ~8 | Temperature sensor configuration and readout |
| TT_PLL_PVT | 0x3020A00 | ~8 | PLL/PVT programmable threshold and status |
| TT_PLL_PVT_CLK_OBSERVE | 0x3020B00 | 3 | Clock counter configuration for observation |

#### Key Registers

| Register | Abs Addr | Access | Description |
|---|---|---|---|
| AWM_GLOBAL_RESOURCE_CONFIGURATION_ENABLES | 0x3020000 | RW | Enable AWM subsystems: bit per domain |
| AWM_GLOBAL_DYNAMIC_SCHEME_0 | 0x3020004 | RW | Dynamic frequency scheme 0 configuration |
| AWM_FREQUENCY0_FCW_INT0 | 0x3020100 | RW | Frequency domain 0 integer FCW word 0 |
| AWM_FREQUENCY0_FCW_INT1 | 0x3020104 | RW | Frequency domain 0 integer FCW word 1 |
| AWM_CGM0_ENABLES | 0x3020280 | RW | Clock gate manager 0: per-unit enable bits |
| AWM_CGM0_OPEN_LOOP | 0x3020284 | RW | CGM0 open-loop clock override |
| DROOP_0_ENABLES | 0x3020500 | RW | Droop detector 0 enable configuration |
| TEMP_SENSOR_CTRL | 0x3020900 | RW | Temperature sensor enable and calibration |
| TT_PLL_PVT_PROGRAMMABLE_THRESHOLD | 0x3020A00 | RW | PVT alert threshold |
| TT_PLL_PVT_CLK_OBSERVE_CLK_CTR_CONFIG | 0x3020B00 | RW | Clock counter window and mux config |

```c
// Example: read AWM global enables and enable AWM
#define AWM_BASE  0x03020000UL
#define AWM_REG(o) (*(volatile uint32_t*)(AWM_BASE+(o)))

uint32_t awm_get_enables(void) {
    return AWM_REG(0x000);  // RESOURCE_CONFIGURATION_ENABLES
}

void awm_enable_freq_domain(int domain) {
    // Each frequency domain enable is a bit in the global enables register
    AWM_REG(0x000) |= (1u << domain);
}
```
...
The Security Monitor Network (SMN) enforces per-tile NoC access permissions and provides address translation, command buffering, inter-tile mailboxes, and die-to-die BAR configuration. Each Tensix tile has one SMN slave instance.

**Base address:** `0x03010000`  
**Sub-blocks:** MST_COMMON, MST_MAIN, ADDR_TRANS, MST_CMD_BUF, SLAVE_MAILBOX_0/1, SLV_BLOCK, SLV_NOC_SEC, SLV_D2D

### §19.2 Mailbox Usage

The SMN provides two mailboxes per tile for inter-tile signaling (separate from the BRISC mailbox system in §3.7):

```c
#define SMN_MBOX0_W  0x03017000UL  // Write-only
#define SMN_MBOX0_R  0x03017008UL  // Read-only
#define SMN_MBOX0_ST 0x03017010UL  // Status: bit[0]=full, bit[1]=empty

// Producer: write message
void smn_send(uint32_t msg) {
    while (*(volatile uint32_t*)SMN_MBOX0_ST & 0x1) {}  // wait empty
    *(volatile uint32_t*)SMN_MBOX0_W = msg;
}

// Consumer: read message
uint32_t smn_recv(void) {
    while (!(*(volatile uint32_t*)SMN_MBOX0_ST & 0x1)) {}  // wait full
    return *(volatile uint32_t*)SMN_MBOX0_R;
}
```

### §19.3 NoC Security Configuration

The SMN enforces which (X,Y) tiles can access which address ranges via the ADDR_TRANS mask table. Each entry has a mask and match: if `(request_addr & mask) == match`, the SMN permits the access.

```c
#define SMN_ADDR_TRANS_BASE  0x03012000UL

// Configure address translation entry N (stride = 0x8)
void smn_set_addr_trans(int n, uint32_t mask, uint32_t match) {
    uint32_t base = SMN_ADDR_TRANS_BASE + n * 0x8;
    *(volatile uint32_t*)(base + 0x0) = mask;   // CFG_N mask word
    *(volatile uint32_t*)(base + 0x4) = match;  // CFG_N match word
}
```
...
The RISC-V External Debug Module (v0.13.2) is accessible via APB at `0x0300A000`. It allows an external debugger to halt/resume individual harts, execute abstract commands, inject code via the program buffer, and perform system bus memory access.

**Key capabilities:**
- Halt/resume any of the 8 RISC-V harts independently (BRISC + TRISC0..2 per tile × tiles)
- GPR/CSR read/write via abstract commands
- Program buffer: up to 16 RISC-V instructions
- System bus access (SBCS/SBADDR/SBDATA): direct memory read/write without halting a hart

### §20.2 Halt and Resume

```c
#define DBG  0x0300A000UL
#define DMCONTROL  (*(volatile uint32_t*)(DBG+0x040))
#define STATUS     (*(volatile uint32_t*)(DBG+0x044))
#define HALTSUMMARY0 (*(volatile uint32_t*)(DBG+0x100))

// Halt hart N (hartsel in bits [25:16] for lo, [15:6] for hi)
void dbg_halt(int hart) {
    DMCONTROL = (1u<<31) | ((hart&0x3FF)<<16) | 1u;  // haltreq + hartsel + dmactive
    while (!(STATUS & (1u<<9))) {}  // wait anyhalted
    DMCONTROL = ((hart&0x3FF)<<16) | 1u;             // clear haltreq
}

void dbg_resume(int hart) {
    DMCONTROL = (1u<<30) | ((hart&0x3FF)<<16) | 1u;  // resumereq + hartsel + dmactive
    while (STATUS & (1u<<9)) {}  // wait until not halted
    DMCONTROL = ((hart&0x3FF)<<16) | 1u;
}
```

### §20.3 System Bus Access

```c
#define SBCS   (*(volatile uint32_t*)(DBG+0x0E0))
#define SBADDR0 (*(volatile uint32_t*)(DBG+0x0E4))
#define SBDATA0 (*(volatile uint32_t*)(DBG+0x0F0))

// Read a 32-bit word via system bus (no halt required)
uint32_t dbg_sb_read32(uint32_t addr) {
    SBCS = (2u<<17) | (1u<<15);  // sbaccess=32bit, sbreadonaddr=1
    SBADDR0 = addr;
    while (SBCS & (1u<<21)) {}   // wait sbusy clear
    return SBDATA0;
}
```
...
The AWM dynamically adjusts clock frequencies and manages power consumption based on workload and thermal conditions. It controls 6 frequency domains, 3 clock gate managers, 4 droop detectors, a temperature sensor, and a PLL/PVT monitor.

**Base address:** `0x03020000`  
**Address range:** `0x03020000` – `0x03020BFF`

### §21.2 Frequency Domain Configuration

Each of the 6 frequency domains has FCW (Frequency Control Word) registers that set the integer and fractional parts of the clock divider:

```c
#define AWM_BASE   0x03020000UL
#define FREQ0_BASE 0x03020100UL  // domain 0
#define FREQ1_BASE 0x03020140UL  // domain 1
// ... stride = 0x40 per domain

// Set frequency domain 0 FCW
void awm_set_freq(int domain, uint32_t fcw_int0, uint32_t fcw_int1) {
    uint32_t base = FREQ0_BASE + domain * 0x40;
    *(volatile uint32_t*)(base + 0x00) = fcw_int0;  // FCW_INT0
    *(volatile uint32_t*)(base + 0x04) = fcw_int1;  // FCW_INT1
}
```

### §21.3 Clock Gate Manager

The CGM controls fine-grained clock gating across all sub-units:

```c
#define CGM0_BASE 0x03020280UL

// Enable all units in CGM0
void awm_cgm_enable_all(void) {
    *(volatile uint32_t*)(CGM0_BASE + 0x00) = 0xFFFFFFFF;  // ENABLES
}

// Force open-loop (bypass CGM gating for debug)
void awm_cgm_open_loop(void) {
    *(volatile uint32_t*)(CGM0_BASE + 0x04) = 0x1;  // OPEN_LOOP
}
```

### §21.4 Temperature Sensor Readout

```c
#define TEMP_SENSOR_BASE 0x03020900UL

// Enable temp sensor and read value
uint32_t awm_read_temperature(void) {
    // Enable sensor
    *(volatile uint32_t*)(TEMP_SENSOR_BASE + 0x00) = 0x1;  // CTRL enable
    // Wait for valid reading (implementation-specific polling)
    for (volatile int i = 0; i < 1000; i++) {}
    // Read temperature code (convert to Celsius per calibration table)
    return *(volatile uint32_t*)(TEMP_SENSOR_BASE + 0x04);
}
```

---

*Document: Trinity N1B0 — NPU Software Engineer's Guide v0.9*
*Date: 2026-03-26*
*§18 source: N1B0_NPU_sfr_v0.5.csv (updated: T6_L1_CSR corrected, IDMA_APB expanded, SMN/DEBUG_MODULE/AWM added)*
