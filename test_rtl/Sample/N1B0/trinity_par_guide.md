...
| `trinity_noc2axi_router_ne_opt` | NOC2AXI+Router NE composite (X=1) | AXI bridge + router, spans Y=4+Y=3 |
| `trinity_noc2axi_router_nw_opt` | NOC2AXI+Router NW composite (X=2) | AXI bridge + router, spans Y=4+Y=3 |
...
│
├── ROUTER-placeholder  (Y=3, X=1 / X=2) — EMPTY gen_block; no module instantiated
│     (router logic is inside NOC2AXI_ROUTER_*_OPT composite tiles above)
│
...
│
└── NOC2AXI  (Y=4)
      ├── trinity_noc2axi_ne_opt          ← partition  (X=0, single-row)
      ├── trinity_noc2axi_router_ne_opt   ← partition  (X=1, dual-row Y=4+Y=3)
      ├── trinity_noc2axi_router_nw_opt   ← partition  (X=2, dual-row Y=4+Y=3)
      └── trinity_noc2axi_nw_opt          ← partition  (X=3, single-row)
```
...
```
    ...
    egress.ack_tgl ◄──────── wire ──────  ingress.ack_tgl   ← direct wire, no sync FF (see §8.4 note)
          │                                  │
...
**RTL audit result (2026-03-20):** No sync-flop cells exist on `req_tgl`, `ack_tgl`, or `data` in the EDC ring RTL. The parameters `DISABLE_SYNC_FLOPS`, `ENABLE_INGRESS_SYNC`, and `ENABLE_EGRESS_SYNC` do **not exist** in the implemented RTL. All inter-node wires are plain combinational pass-through (`tt_edc_intf_connector`). Do not ask the P&R tool to insert synchronizer cells on these nets.

**CDC safety basis (Samsung 5nm, N1B0):** The self-throttling toggle protocol guarantees that `req_tgl` changes at most once per full ring round-trip. The hardware-minimum round-trip time is ≈ 20 ns (10 tiles × 1 ns each + MCPDLY × 1 ns), giving an effective toggle rate ceiling of **50 MHz**. On Samsung 5nm (τ ≈ 10 ps), the MTBF at 50 MHz worst case exceeds **10²⁵ years** per crossing. `set_false_path` is sufficient and complete; no sync cell insertion and no proximity (max_distance) constraint are needed. See `guide_max_distance_constraints.md §15` for the full MTBF calculation.

```tcl
# EDC ring toggle lines: req_tgl, ack_tgl, data[15:0], data_p
# Protocol guarantees data stability — no metastability risk on Samsung 5nm
# (self-throttling: f_data ≤ 50 MHz ceiling; MTBF >> 10^25 years at τ=10ps)
# Correct treatment: set_false_path. Do NOT set MCP or insert 2FF sync cells.

set_false_path -from [get_clocks NOCCLK] -through [get_nets *edc*req_tgl*] \
               -to   [get_clocks AICLK]

set_false_path -from [get_clocks NOCCLK] -through [get_nets *edc*data*] \
               -to   [get_clocks AICLK]

set_false_path -from [get_clocks AICLK]  -through [get_nets *edc*ack_tgl*] \
               -to   [get_clocks NOCCLK]

# Apply the same false_path for any other inter-tile edc ring cross-clock wires
set_false_path -through [get_nets *edc_egress_intf*] -to [get_clocks {AICLK NOCCLK}]
set_false_path -through [get_nets *edc_loopback*]    -to [get_clocks {AICLK NOCCLK}]
```

> **CDC waiver:** Add a CDC waiver entry for all `edc*req_tgl`, `edc*ack_tgl`, `edc*data` nets crossing NOCCLK/AICLK. Waiver justification: self-throttling protocol (see §3.5.4 of EDC_HDD_V0.5.md). Do not suppress the waiver at chip level — document it in the CDC sign-off sheet.

> **Risk note for non-5nm process ports:** If ported to a node with τ > 50 ps, recalculate MTBF using the formula in `guide_max_distance_constraints.md §15.1` before tapeout. At τ = 50 ps and 50 MHz toggle rate, MTBF ≈ 26 seconds — insufficient.
...
| **EDC toggle CDC** | **`set_false_path` on req_tgl/ack_tgl/data across NOCCLK↔AICLK. NO 2FF insertion, NO proximity constraint (Samsung 5nm). Add CDC waiver with MTBF justification.** | **§8.4, EDC_HDD_V0.5 §3.5.4** |
...
*End of Document — Version 0.6 (2026-03-24)*
*v0.6: §8.4 corrected — RTL audit confirmed DISABLE_SYNC_FLOPS/ENABLE_INGRESS_SYNC/ENABLE_EGRESS_SYNC parameters do not exist in used_in_n1 RTL. Replaced MCP-based SDC with `set_false_path` on all EDC toggle lines (req_tgl/ack_tgl/data). CDC safety basis documented: Samsung 5nm τ≈10ps, self-throttling protocol limits f_data≤50MHz, MTBF>10²⁵ years. Added CDC waiver note and non-5nm risk warning. Fixed §8.4 CDC diagram (ack_tgl = direct wire, not 2FF sync). Added EDC CDC row to §10 checklist. Cross-referenced guide_max_distance_constraints.md §15 for MTBF derivation.*
