...
**Document:** router_decode_HDD_v0.5.md
...
| v0.4 | 2026-03-17 | (RTL-derived) | Added Section 4.6 routing pseudo code (all modes in one block) |
| v0.5 | 2026-03-17 | (RTL-derived) | Expanded Section 4.3 dynamic routing with carried-list mechanism, response routing, force_dim interaction, and DOR vs dynamic comparison table |
| v0.5.1 | 2026-03-19 | (N1B0 update) | Updated §2 tile types and grid to N1B0 enum; §10/§11 N1B0 composite tile notes; §14 EP table; §18 RTL file index |
...
4. [Routing Decision Logic](#4-routing-decision-logic)
   ...
   - 4.3 [Dynamic Routing Override](#43-dynamic-routing-override)
     - 4.3.1 [Overview and Motivation](#431-overview-and-motivation)
     - 4.3.2 [The Carried Routing List](#432-the-carried-routing-list)
     - 4.3.3 [Per-Hop Execution](#433-per-hop-execution)
     - 4.3.4 [Dynamic vs DOR Comparison](#434-dynamic-vs-dor-comparison)
     - 4.3.5 [force_dim_routing Flag](#435-force_dim_routing-flag)
   - 4.4 [Port Mask and Orientation Mapping](#44-port-mask-and-orientation-mapping)
   - 4.5 [Broadcast Routing](#45-broadcast-routing)
   - 4.6 [Routing Logic — Complete Pseudo Code](#46-routing-logic--complete-pseudo-code)
...
```
  X=0               X=1                      X=2                      X=3
Y=4  NOC2AXI_NE_OPT NOC2AXI_ROUTER_NE_OPT  NOC2AXI_ROUTER_NW_OPT  NOC2AXI_NW_OPT  (North — AXI/DRAM, EP 4/9/14/19)
Y=3  DISPATCH_E      ROUTER(placeholder)     ROUTER(placeholder)     DISPATCH_W      (Dispatch row, EP 3/8/13/18)
Y=2  TENSIX          TENSIX                  TENSIX                  TENSIX          (EP 2/7/12/17)
Y=1  TENSIX          TENSIX                  TENSIX                  TENSIX          (EP 1/6/11/16)
Y=0  TENSIX          TENSIX                  TENSIX                  TENSIX          (EP 0/5/10/15)
```

> **Note:** ROUTER_NE_OPT and ROUTER_NW_OPT are composite dual-row tiles spanning Y=4+Y=3.
...
|------|-------|-------------|
| `TENSIX` | 3'd0 | AI compute core (Y=0..2, all columns) |
| `NOC2AXI_NE_OPT` | 3'd1 | AXI bridge NE corner (X=0, Y=4) |
| `NOC2AXI_ROUTER_NE_OPT` | 3'd2 | AXI bridge + Router composite (X=1, Y=4+Y=3) |
| `NOC2AXI_ROUTER_NW_OPT` | 3'd3 | AXI bridge + Router composite (X=2, Y=4+Y=3) |
| `NOC2AXI_NW_OPT` | 3'd4 | AXI bridge NW corner (X=3, Y=4) |
| `DISPATCH_E` | 3'd5 | Dispatch Engine East (X=3, Y=3) |
| `DISPATCH_W` | 3'd6 | Dispatch Engine West (X=0, Y=3) |
| `ROUTER` | 3'd7 | Router placeholder (X=1,2 Y=3 — empty gen_block; router logic is inside NOC2AXI_ROUTER_*_OPT) |
...
DOR always takes the same shortest XY path for a given src→dst pair. Dynamic routing lets SW pre-compute **any arbitrary path** and embed the full per-hop decisions directly in the flit header. Use cases include:

- Routing around a congested or faulty link (non-minimal detour)
- Delivering to non-contiguous node subsets (custom multicast)
- Load balancing across multiple equal-cost paths

#### 4.3.2 The Carried Routing List

When `cmd_dyna_routing_en=1`, the entire routing map for every node in the mesh is **embedded in the head flit header** as `noc_dyn_routing_header_t`. Each node gets a 5-bit slot.

**RTL definition (`tt_noc_pkg.sv`):**
```systemverilog
localparam DYNAMIC_ROUTING_LIST_WIDTH = 2 * NOC_HDR_ECC_SEG_DATA_WIDTH;
//   = 2 × (116 × 4) = 928 bits

typedef struct packed {
    logic [DYNAMIC_ROUTING_LIST_WIDTH/2-1:0] list;
} noc_dyn_routing_info_t;

typedef struct packed {
    noc_dyn_routing_info_t port_2;   // upper half of list
    noc_dyn_routing_info_t port_1;   // lower half of list
} noc_dyn_routing_header_t;
```

**Slot layout inside the 928-bit list:**
```
For each node at (local_x, local_y):
  offset = local_y * noc_x_size + local_x
  slot   = routing_list[offset * 5 +: 5]
         = {X-, Y-, X+, Y+, NIU}     // 5-bit one-hot port decision
```

For Trinity 4×5: 20 nodes × 5 bits = 100 bits used out of 928 bits.

#### 4.3.3 Per-Hop Execution

At each router hop the routing module reads and consumes its own slot (`tt_niu_output_head_flit_routing.sv:294-310`):

```systemverilog
// Step 1 — read this node's port decision from the carried list
{packet_fwd_x_neg, packet_fwd_y_neg, packet_fwd_x_pos, packet_fwd_y_pos, packet_fwd_niu}
    = req_dynamic_routing_list[dyn_list_offset * NUM_ROUTER_PORTS +: NUM_ROUTER_PORTS];

// Step 2 — build response routing list:
//   overwrite this node's slot with NIU=1 so response is delivered here
dyn_routing_resp_packet_fwd_niu = 1'b1;
resp_dynamic_routing_list[dyn_list_offset * NUM_ROUTER_PORTS +: NUM_ROUTER_PORTS]
    = {0, 0, 0, 0, dyn_routing_resp_packet_fwd_niu};   // → {X-=0,Y-=0,X+=0,Y+=0,NIU=1}

o_resp_dyn_routing_header = updated list with this node's slot overwritten
```

**Request packet flow:**
```
SW embeds full 928-bit routing list in head flit
    ↓
Node (0,0): reads slot[0*5+:5] → forward X+
    ↓
Node (1,0): reads slot[1*5+:5] → forward Y+
    ↓
Node (1,1): reads slot[6*5+:5] → forward Y+
    ↓
... continues until NIU bit is set in a node's slot
```

**Response packet flow:**
```
Each visited node had its slot overwritten to NIU=1 in the response list
→ response packet delivered locally at every visited node
→ enables scatter/multicast with per-node responses
```

#### 4.3.4 Dynamic vs DOR Comparison

| | DOR | Dynamic Routing |
|---|---|---|
| **Path decided** | On-the-fly at each hop | Pre-computed by SW, embedded in header |
| **Path shape** | Always minimal XY (or YX) | Arbitrary — any port, any order, non-minimal |
| **Header overhead** | Zero | 928-bit routing list in every head flit |
| **Use case** | Normal unicast / broadcast | Non-minimal, load-balanced, custom multicast |
| **Deadlock** | Free by construction | SW must ensure no circular channel dependency |
| **Response** | Returns to original sender via DOR | Each visited node receives response at its local NIU |

#### 4.3.5 `force_dim_routing` Flag

Setting `cmd_force_dim_routing=1` while `cmd_dyna_routing_en=1` switches the current packet back to DOR **without changing the ATT routing table entry**:

```systemverilog
// tt_niu_output_head_flit_routing.sv
wire force_dim_routing  = i_force_dim_routing;
wire dyn_routing_packet = i_dyn_routing_en;

if (dyn_routing_packet AND NOT force_dim_routing):
    // use carried routing_list[dyn_list_offset]
else:
    // use DOR logic (XY or YX)
```

Also note: tendril and mesh-boundary detection is **disabled** when either `force_dim` or `dyn_routing` is active:
```systemverilog
wire local_in_mesh          = force_dim_routing || dyn_routing_packet || (in_mesh_bounds);
wire local_is_vertical_tendril   = !force_dim_routing && !dyn_routing_packet && (...);
wire local_is_horizontal_tendril = !force_dim_routing && !dyn_routing_packet && (...);
```
Dynamic routing and forced DOR both bypass tendril re-entry logic entirely.
...
### 4.6 Routing Logic — Complete Pseudo Code

The following is derived directly from `tt_niu_output_head_flit_routing.sv`. All five routing modes are shown in one block.

```
function ROUTE(pkt, local_x, local_y):

    dest_x = pkt.x_coord;  dest_y = pkt.y_coord
    in_mesh   = (mesh_start_x <= local_x <= mesh_end_x) AND
                (mesh_start_y <= local_y <= mesh_end_y)
    h_tendril = (local_x < mesh_start_x OR local_x > mesh_end_x)
    v_tendril = (local_y < mesh_start_y OR local_y > mesh_end_y)
    umd_x     = clamp(dest_x, mesh_start_x, mesh_end_x)   // mesh-boundary-clamped target
    umd_y     = clamp(dest_y, mesh_start_y, mesh_end_y)

    // ── Select output direction ───────────────────────────────────────────

    if dyn_routing AND NOT force_dim:
        offset = local_y * noc_x_size + local_x
        {X-, Y-, X+, Y+, NIU} = routing_list[offset]           // table lookup per node

    elif brcst:
        if   local outside rectangle:  unicast toward rectangle boundary (X or Y)
        elif local inside rectangle:   fan out both ways along trunk axis + NIU

    elif force_dim OR in_mesh:
        target_x = force_dim ? dest_x : umd_x                  // use clamped target
        target_y = force_dim ? dest_y : umd_y                  //   when inside mesh
        if   dest == local:            NIU
        elif X matched (or YX order):  Y+ / Y-  toward target_y
        else:                          X+ / X-  toward target_x  // X first (default)

    else:  // tendril — re-enter mesh boundary first
        if h_tendril AND local_y != dest_y:  X+ / X-  toward mesh boundary
        if v_tendril AND local_x != dest_x:  Y+ / Y-  toward mesh boundary

    // ── Rotate logical → physical ports by tile orientation ──────────────

    port_mask[NIU, Y+, X+, Y-, X-] = rotate(fwd, orientation)  // 0/90/180/270/FLIP
    return port_mask
```

**Mode selection summary:**

| Priority | Condition | Routing Mode | RTL state |
|----------|-----------|--------------|-----------|
| 1 | `dyn_routing_en=1` AND `force_dim=0` | Dynamic table lookup | `dyn_list_offset * 5` bits from routing list |
| 2 | `brcst_en=1` | Broadcast state machine | `NOC_BRCST_X/Y_UNICAST → MCAST → LAST_PATH` |
| 3 | `force_dim=1` OR `in_mesh` | Dimension-order routing (DOR) | X-first (default) or Y-first |
| 4 | tendril node | Re-enter mesh first, then DOR | horizontal: go X; vertical: go Y |

**Orientation rotation table (`NOC_ORIENT_*`):**

| Orientation | Physical port order `[X-, Y-, X+, Y+, NIU]` |
|-------------|----------------------------------------------|
| `0°` (default, Trinity) | `{X-, Y-, X+, Y+, NIU}` |
| `90° CCW` | `{Y+, X-, Y-, X+, NIU}` |
| `180°` | `{X+, Y+, X-, Y-, NIU}` |
| `270° CCW` | `{Y-, X+, Y+, X-, NIU}` |
| `FLIP+0°` | `{X+, Y-, X-, Y+, NIU}` |
| `FLIP+90°` | `{Y+, X+, Y-, X-, NIU}` |
| `FLIP+180°` | `{X-, Y+, X+, Y-, NIU}` |
| `FLIP+270°` | `{Y-, X-, Y+, X+, NIU}` |
...
AXI address [55:0] (56-bit, NPU_IN_ADDR_W=56):
 ┌──────────┬──────────┬──────────┬────────────────────────────┐
 │ [55:52]  │ [51:46]  │ [45:40]  │ [39:0]                     │
 │ Reserved │  Y coord │  X coord │  Local address (40-bit)    │
 │  (4-bit) │  (6-bit) │  (6-bit) │                            │
 └──────────┴──────────┴──────────┴────────────────────────────┘
```
...
AXI address [53:0] layout (SW programming convention):
 ┌─────────────────────┬────────────────┬──────────────────────┐
 │ [53:27]             │ [26:22]        │ [21:0]               │
 │  Reserved (27-bit)  │  tensix_id     │  Local addr (22-bit) │
 │                     │  (5-bit)       │  (4MB per tensix)    │
 └─────────────────────┴────────────────┴──────────────────────┘
```
...
tensix_id = y_coord * SizeX + x_coord
          = y_coord * 4    + x_coord    (for Trinity 4×5 grid)
```

> **Important:** This is row-major indexing by Y then X. It differs from `EndpointIndex = x * SizeY + y` (column-major, used in `trinity_pkg.sv`). For Trinity 4×5:
> - `tensix_id` range: 0–19 (5 rows × 4 columns)
> - Example: (x=1, y=2) → `tensix_id = 2*4+1 = 9`; `EndpointIndex = 1*5+2 = 7`
>
...
Each of the 4 AXI slave ports (NPU SI0–SI3) is allocated **128 MB** of address space in the system memory map. These ports connect to the NOC2AXI bridge tiles at Y=4.
...
> **N1B0 note:** In N1B0, `trinity_router` is NOT a standalone synthesis partition. It is instantiated INSIDE `trinity_noc2axi_router_ne_opt` (X=1) and `trinity_noc2axi_router_nw_opt` (X=2) as the Y=3 sub-module. The `gen_router` generate blocks at (X=1,Y=3) and (X=2,Y=3) in `trinity.sv` are **empty**. REP_DEPTH_LOOPBACK=6, REP_DEPTH_OUTPUT=4 are N1B0-specific register-slice depths added inside `trinity_router` for timing closure.
...
> **N1B0 NIU tiles:** Four variants — `trinity_noc2axi_ne_opt` (X=0,Y=4, EP=4), `trinity_noc2axi_router_ne_opt` (X=1, EP=9/8, dual-row), `trinity_noc2axi_router_nw_opt` (X=2, EP=14/13, dual-row), `trinity_noc2axi_nw_opt` (X=3,Y=4, EP=19). The composite tiles use port prefix `noc2axi_*` for Y=4 signals and `router_*` for Y=3 signals.
...
Where X ∈ [0,3], Y ∈ [0,4] (for a 4×5 grid with SizeX=4, SizeY=5).

**Hierarchy verification status:**

| Level | Instance/Generate Name | Verified | Source |
|-------|----------------------|----------|--------|
| `trinity` | top module | ✅ | `trinity.sv` |
| `gen_x[X]` | generate block | ✅ | `trinity.sv:182` |
| `gen_y[Y]` | generate block | ✅ | `trinity.sv:183` |
| `gen_tensix` | generate block | ✅ | `trinity.sv:762` |
| `tt_tensix_with_l1` | instance | ✅ | `trinity.sv:772` |
| `overlay_noc_wrap` | instance | ⚠️ plausible | `tt_overlay_noc_wrap.sv` in filelist (encrypted) |
| `overlay_noc_niu_router` | instance | ⚠️ plausible | `tt_overlay_noc_niu_router.sv` in filelist (encrypted) |
| `tt_trinity_noc_niu_router_inst` | instance | ⚠️ plausible | `tt_trin_noc_niu_router_wrap.sv` in filelist (encrypted) |
| `noc_niu_router_inst` | instance | ⚠️ plausible | `tt_noc_niu_router.sv` in filelist (encrypted) |
| `.niu.mst_if.reg_mst_if.reg_intf[0].risc_reg_if` | sub-path | ⚠️ unverifiable | All in encrypted RTL |
...
**Coordinate programming example** — send to node (x=2, y=3):
```
...
| 1 | 1 | TENSIX | 6 |
| 1 | 2 | TENSIX | 7 |
| 1 | 3 | NOC2AXI_ROUTER_NE_OPT/Router (sub-module of composite) | 8 |
| 1 | 4 | NOC2AXI_ROUTER_NE_OPT | 9 |
| 2 | 0 | TENSIX | 10 |
| 2 | 1 | TENSIX | 11 |
| 2 | 2 | TENSIX | 12 |
| 2 | 3 | NOC2AXI_ROUTER_NW_OPT/Router (sub-module of composite) | 13 |
...
| `trinity_noc2axi_router_ne_opt.sv` | `used_in_n1/rtl/trinity_noc2axi_router_ne_opt.sv` | NOC2AXI+Router composite NE (X=1, dual-row Y=4+Y=3) — N1B0 only |
| `trinity_noc2axi_router_nw_opt.sv` | `used_in_n1/rtl/trinity_noc2axi_router_nw_opt.sv` | NOC2AXI+Router composite NW (X=2, dual-row Y=4+Y=3) — N1B0 only |
...
1. **X/Y coordinates swapped** — Writing Y into the X field and vice versa in `TARG_ADDR_HI`. Causes the packet to route to the wrong node.

2. **Address exceeds 56-bit range** — The NOC2AXI tile drops bits [63:56] of the NoC address. Any value in those bits is silently truncated.

3. **Reserved bits non-zero in default mode** — Bits [55:52] of the AXI address are reserved in gasket mode. Non-zero values may cause unexpected gasket behavior.

4. **ATT disabled but address not in gasket format** — If ATT is disabled and the AXI address is not formatted as [Y|X|local], the gasket extracts incorrect X/Y coordinates.

5. **Wrong `tensix_id` formula** — In ATT mode, using `x * SizeY + y` (EndpointIndex) when the mask table was programmed using `y * 4 + x` (TT convention), or vice versa.

6. **VC mismatch** — Sending a response packet on a unicast request VC (0–7) can cause credit class violations.

### 19.3 Inbound Bus Hang Conditions

Inbound AXI transactions (host → NoC) that target an address with no responding NoC endpoint can cause AXI bus hangs. Conditions that lead to this:

- Target tile is harvested (coordinate = `NOC_HARVEST_ID`) — packet dropped, no AXI response generated
- ATT miss (no mask table entry matches the address) — packet not translated, endpoint undefined
- Destination tile is powered down or in reset — packet delivered but no AXI response returned
