# N1B0 NPU Hardware Design Document v0.91
...
TRISC3 has three memory interfaces inside the tile:

| Interface | Width | Target | Purpose |
|-----------|-------|--------|---------|
| Instruction fetch | 32-bit | L1 partition (ICACHE region) | TRISC3 program instructions |
...
While the FPU writes results for tile N into Buffer B, the pack engine simultaneously reads Buffer A (containing results for tile N−1) and writes them to L1 or the NoC. When tile N compute is complete, a **dest_toggle** hardware event fires, signaling TRISC3 (and TRISC0) to swap which buffer is the "read" buffer and which is the "write" buffer.
...
```
  ...
  │  L1 SRAM    │ SRAM macro    │ ai_clk   │ 3 MB       │  36 MB            │
...
```
  ...
  ├─────────────────────┼────────────────────┼──────────────────────┤
  │  Macros per cluster │  128                │  512                 │
  │  Macro size         │  12 KB (768×69b)    │  12 KB (768×69b)     │
...
|--------------|-----------------------|-------------------------|
| ai_clk       | T6 L1 SRAM            | 6,144 macros (512×12)   |
