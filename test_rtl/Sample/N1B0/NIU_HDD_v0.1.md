...
**Primary Sources:** `trinity_noc2axi_nw_opt.sv`, `trinity_noc2axi_ne_opt.sv`, `trinity_noc2axi_router_ne_opt.sv`, `trinity_noc2axi_router_nw_opt.sv`, `trinity_pkg.sv`, `tt_noc2axi_pkg` (external)
**Audience:** Verification Engineers · Software Engineers · Hardware Engineers

| Version | Date | Author | Changes |
|---------|------|--------|---------|
| v0.1 | 2026-03-17 | (RTL-derived) | Initial release |
| v0.2 | 2026-03-19 | (N1B0 update) | N1B0: 4 NIU variants (composite dual-row tiles at X=1,2), RDATA FIFO depth 512, per-column clocks, cross-row internal wiring |
...
The core bridge logic is contained in the external module `tt_noc2axi`. The four N1B0 Trinity wrapper modules add:
...
```
                      trinity_noc2axi_{ne,router_ne,router_nw,nw}_opt
  ┌────────────────────────────────────────────────────────────────────┐
  │                                                                    │
...
```
    x=0                   x=1                          x=2                          x=3
  ┌──────────────────┬──────────────────────────┬──────────────────────────┬──────────────────┐
y=│ noc2axi_ne_opt   │ noc2axi_router_ne_opt    │ noc2axi_router_nw_opt   │ noc2axi_nw_opt   │  ← NIU row (Y=4)
4 │  EP=4 (corner)   │  EP=9 (Y=4+Y=3 dual-row) │  EP=14 (Y=4+Y=3 dual-row)│  EP=19 (corner)  │
  ├──────────────────┼──────────────────────────┼──────────────────────────┼──────────────────┤
y=│ dispatch_e       │ ROUTER (placeholder)     │ ROUTER (placeholder)    │ dispatch_w       │  ← Dispatch row
3 │                  │ (empty gen_block)        │ (empty gen_block)       │                  │
  ├──────────────────┼──────────────────────────┼──────────────────────────┼──────────────────┤
...
NIU tiles occupy the north row (y=4) and act as **boundary nodes** — the NORTH port is permanently disabled (VC buffer depth = 0, flit inputs tied to `'0`). Note that composite tiles (X=1,2) physically span Y=4+Y=3 (dual-row modules).

---

## 4. NIU Variants

Four RTL wrappers are used depending on grid column position.

| Variant | File | Grid Position | Active NoC Ports | EP index |
|---------|------|---------------|------------------|----------|
| `trinity_noc2axi_ne_opt` | `trinity_noc2axi_ne_opt.sv` | Y=4, X=0 | SOUTH, WEST | 4 |
| `trinity_noc2axi_router_ne_opt` | `trinity_noc2axi_router_ne_opt.sv` | Y=4+Y=3, X=1 (dual-row) | EAST+SOUTH+WEST (Y=4); N+E+S+W (Y=3 router) | 9 (Y=4), 8 (Y=3) |
| `trinity_noc2axi_router_nw_opt` | `trinity_noc2axi_router_nw_opt.sv` | Y=4+Y=3, X=2 (dual-row) | EAST+SOUTH+WEST (Y=4); N+E+S+W (Y=3 router) | 14 (Y=4), 13 (Y=3) |
| `trinity_noc2axi_nw_opt` | `trinity_noc2axi_nw_opt.sv` | Y=4, X=3 | SOUTH, EAST | 19 |

The composite variants (`trinity_noc2axi_router_ne/nw_opt`) are dual-row modules that physically span Y=4 (NOC2AXI level) and Y=3 (Router level). Internally they instantiate `trinity_noc2axi_n_opt` (Y=4) and `trinity_router` (Y=3) with explicit cross-row flit wiring. See `N1B0_NOC2AXI_ROUTER_OPT_HDD_v0.1.md` for the internal architecture.
...
### 5.2.1 N1B0 RDATA FIFO Depth

In N1B0, the AXI slave read-data FIFO depth is define-selectable (32/64/128/256/**512**/1024). Default is 512. SRAM wrapper files in `used_in_n1/rtl/noc2axi_slv_rddata_fifo_depth_*.sv` cover all depths.
...
```
trinity_noc2axi_{ne,nw}_opt   (corner tiles)               (trinity_noc2axi_*.sv)
...
```
trinity_noc2axi_router_ne_opt / trinity_noc2axi_router_nw_opt   (composite dual-row)
├── trinity_noc2axi_n_opt   (Y=4 sub-module — noc2axi_* port prefix)
│   ├── tt_noc2axi                               (AXI bridge — same as corner tiles)
│   ├── tt_edc1_biu_soc_apb4_wrap                (EDC BIU — APB CSR access for Y=4 EDC node only)
│   ├── tt_edc1_noc_sec_controller               (EDC node 192 — mesh config override)
│   └── tt_noc_overlay_edc_repeater              (EDC ring output → internal cross-row wire)
│
└── trinity_router          (Y=3 sub-module — router_* port prefix)
    ├── tt_disp_eng_noc_niu_router               (NoC router with ATT/EDC)
    └── EDC ring segment for Y=3 router nodes

Internal cross-row connections (hardwired inside composite module):
  noc2axi_o_flit_out_req_south   → router_i_flit_in_req_north   (NoC flit south→north)
  router_o_flit_out_req_north    → noc2axi_i_flit_in_req_south  (NoC flit north→south)
  noc2axi_clock_routing_out      → router_clock_routing_in       (9 clock fields)
  noc2axi_edc_egress_intf        → router_edc_ingress_intf       (EDC forward chain)
  router_loopback_edc_egress_intf → noc2axi_loopback_edc_ingress_intf  (EDC loopback)
```
...
**N1B0:** `i_ai_clk` and `i_dm_clk` are per-column arrays at the `trinity` top (width=4). Corner tiles (X=0,3) drive `clock_routing_out[x][4]`. Composite tiles (X=1,2) output clock routing at Y=3 via `router_o_ai_clk`/`router_o_dm_clk` → `clock_routing_out[x][3]`.
...
No gating or buffering — pure wire for physical abutment. Note: the above applies to corner tiles (`trinity_noc2axi_ne_opt.sv`). Composite tiles output clocks from `router_o_*` ports at Y=3, routing the clock chain output through the Y=3 router sub-module.

### 7.3 Reset Signal Routing

| Reset Signal | Behavior | RTL (line numbers from trinity_noc2axi_ne_opt.sv) |
|---|---|---|
...
| `AXI_SLV_RD_RDATA_FIFO_DEPTH` | **512** (N1B0 default; define-selectable: 32/64/128/256/512/1024) | NOC2AXI read data return FIFO depth |
...
Composite tiles use `used_in_n1/rtl/trinity_noc2axi_router_ne/nw_opt.sv`.
...
tt_noc2axi #(
    ...
    .AXI_SLV_RD_RDATA_FIFO_DEPTH  (512),   // NOC2AXI read data FIFO depth (N1B0 default)
