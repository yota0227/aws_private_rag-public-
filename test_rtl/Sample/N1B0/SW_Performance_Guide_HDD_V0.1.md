...
**Date:** 2026-03-18
**N1B0 update:** 2026-03-19 — L1 capacity corrected to 768 KB/tile (N1B0 trin L1 config: 16 banks × 3072×128b = 48 KB/bank).
...
│
├── L1 SRAM  (768 KB/tile in N1B0 — 16 banks × 3072×128b = 48 KB/bank; baseline ~1.0 MB/tile with Mimir L1 config; ai_clk, shared by all 4 T6s)
...
| L1 SRAM | **768 KB** (N1B0 trin config: 16 banks × 48 KB) | ai_clk | 128b/bank/cycle, 16 banks | 4–6 cycles | all 4 T6 |
...
```
L1 structure (N1B0 trin config — 768 KB/tile):
  16 banks × 3072×128b = 48 KB/bank = 768 KB total
  (Baseline Mimir config: 64 macros, 4 sbank × 4 bank × 4 sub_bank, each 1024×128b = 16 KB)
