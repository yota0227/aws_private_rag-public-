...
10. [Mechanism 6: ISO_EN Tile Output Isolation (N1B0)](#10-mechanism-6-iso_en-tile-output-isolation-n1b0)
11. [Clock Domains and CDC](#11-clock-domains-and-cdc)
12. [Reset Architecture](#12-reset-architecture)
13. [SDC / Timing Notes](#13-sdc--timing-notes)
14. [How to Operate Harvest in RTL (Step-by-Step)](#14-how-to-operate-harvest-in-rtl-step-by-step)
15. [Register Interface Summary](#15-register-interface-summary)
16. [Key RTL Paths and File Locations](#16-key-rtl-paths-and-file-locations)
17. [Notes for Specific Audiences](#17-notes-for-specific-audiences)
...
| `NOC2AXI_NE_OPT` | 3'd1 | No (X=0, Y=4, always present) |
| `NOC2AXI_ROUTER_NE_OPT` | 3'd2 | No (X=1, Y=4+Y=3 composite, always present) |
| `NOC2AXI_ROUTER_NW_OPT` | 3'd3 | No (X=2, Y=4+Y=3 composite, always present) |
| `NOC2AXI_NW_OPT` | 3'd4 | No (X=3, Y=4, always present) |
| `DISPATCH_E` | 3'd5 | No (X=3, Y=3, always present) |
| `DISPATCH_W` | 3'd6 | No (X=0, Y=3, always present) |
| `ROUTER` | 3'd7 | N/A (empty placeholder at X=1,2,Y=3 — no module instantiated) |
...
```
  ...
  (N)   │ NE_OPT (X=0) │ ROUTER_NE_OPT (X=1)      │ ROUTER_NW_OPT (X=2)      │ NW_OPT (X=3) │
        │ single-row   │ dual-row (Y=4+Y=3)       │ dual-row (Y=4+Y=3)       │ single-row   │
        ├──────────────┼──────────────┼──────────────┼──────────────┤
...
```
  ...
  │                                                                                      │
  │   ┌──────────────────────────────────────────────────────────────────────┐          │
  │   │ ISO_EN isolation (N1B0 addition)                                     │          │
  │   │  ISO_EN[x + 4*y] = 1 → isolation cells force tile outputs to safe   │          │
  │   │  values; prevents bus contention on shared wires during power-down   │          │
  │   └──────────────────────────────────────────────────────────────────────┘          │
  └──────────────────────────────────────────────────────────────────────────────────────┘
```
...
**N1B0:** `i_ai_clk` and `i_dm_clk` are per-column arrays (`[SizeX-1:0]`). Each column has independent AI and DM clocks. The composite NOC2AXI_ROUTER tiles always pass clocks through to Y=3 regardless of harvest state.
...
## 10. Mechanism 6: ISO_EN Tile Output Isolation (N1B0)

### 10.1 Overview

N1B0 adds a sixth harvest mechanism: output isolation via `ISO_EN[11:0]` at the `trinity` top level. Isolation cells at each Tensix tile's output boundary are controlled by this signal. When asserted, the isolation cells force all tile outputs (NoC flit lines, bus outputs, interrupt wires) to a safe logic level, preventing bus contention when the tile is powered down.

### 10.2 ISO_EN Bit Map

`ISO_EN[x + 4*y]` controls isolation for the Tensix tile at column x, row y:

```
ISO_EN bit mapping:
  [3:0]  → Y=0 row: T6[0][0], T6[1][0], T6[2][0], T6[3][0]
  [7:4]  → Y=1 row: T6[0][1], T6[1][1], T6[2][1], T6[3][1]
  [11:8] → Y=2 row: T6[0][2], T6[1][2], T6[2][2], T6[3][2]
```

Only TENSIX tiles (Y=0..2) have ISO_EN bits. NOC2AXI, Dispatch, and Router tiles are never isolated.

### 10.3 ISO Cell Types

From `trinity_par_guide.md §9A`:

| Signal group | ISO type | Safe value |
|---|---|---|
| `flit_out_req[x][y][dir]` (×4 dirs, outbound NoC flits) | AND | 0 (no flit) |
| `flit_in_resp[x][y][dir]` (×4 dirs, credit backpressure) | AND | 0 (no credits) |
| `t6_to_de[x][y]` (Tensix→Dispatch feedback) | AND | 0 |
| L1 async FIFO feedback signals | AND | 0 |
| Interrupt outputs | AND | 0 |

### 10.4 When to Assert ISO_EN

ISO_EN must be asserted **together with reset isolation** (Mechanism 1) for a complete harvest. Sequence:
1. Assert `i_tensix_reset_n[getTensixIndex(x,y)] = 0` (Mechanism 1)
2. Assert `ISO_EN[x + 4*y] = 1` (Mechanism 6)
3. Program NoC mesh config (Mechanism 3)
4. EDC bypass is automatic (Mechanism 4)
5. Update ATT if needed

ISO_EN alone without reset isolation is not a valid harvest state.

---

## 11. Clock Domains and CDC

### 11.1 Clock Domains Summary
...
### 12.1 Active-Low Reset Hierarchy
...
### 12.2 Reset Sequencing for Harvest
...
### 13.1 Multi-Cycle and False Path Constraints for Harvest
...
### 13.3 Clock Domain Crossings — Exceptions
...
**Step 2a. (N1B0 only) Assert `ISO_EN[x + 4*y] = 1` for the harvested tile.**
...
### 14.2 Verifying Harvest is Active
...
## 15. Register Interface Summary

### 15.1 APB Register Interface (per NOC2AXI tile)
...
### 15.2 EDC APB Interface (per NOC2AXI tile)
...
### 15.3 EDC noc_sec_controller Configuration (Harvest-Specific)
...
### 15.4 ISO_EN Register

`ISO_EN[11:0]` — trinity top-level input — isolation enable per Tensix tile (x + 4*y). Bit `[x + 4*y]` asserts AND-type isolation cells on all outputs of the Tensix tile at grid position (x, y). Only bits [11:0] are valid (12 Tensix tiles, Y=0..2 × X=0..3).

### 15.5 Interrupt Outputs
...
## 16. Key RTL Paths and File Locations

### 16.1 Top-Level and Package Files
...
### 16.2 NOC2AXI Boundary Tiles (Harvest Config Entry Point)
...
| [used_in_n1/rtl/trinity_noc2axi_router_ne_opt.sv](trinity_noc2axi_router_ne_opt.sv) | x=1, y=4+y=3 (composite) | East + South + West (Y=4); all dirs (Y=3) |
| [used_in_n1/rtl/trinity_noc2axi_router_nw_opt.sv](trinity_noc2axi_router_nw_opt.sv) | x=2, y=4+y=3 (composite) | East + South + West (Y=4); all dirs (Y=3) |

All four contain identical harvest configuration logic (EDC noc_sec_controller + mesh override mux).

### 16.3 Other Key Tile RTL (Encrypted, under `rtl/enc/`)
...
### 16.6 Memory / ECC (not harvest-specific, but relevant for L1 testing)
...
## 17. Notes for Specific Audiences

### 17.1 Verification Engineer
...
- N1B0: Verify ISO_EN[x+4*y] assertion prevents bus contention on shared wires when tile is power-gated.
...
### 17.2 Software / Firmware Engineer
...
### 17.3 Physical Implementation Engineer
...
- **N1B0:** ISO_EN isolation cells must be placed at the tile output boundary. AND-type cells are used; placement must be adjacent to the tile's output drivers to minimize routing of un-isolated signals.

### 17.4 HW/Microarchitecture Engineer
...
7. **N1B0 ISO_EN (Mechanism 6)** — ISO_EN is a separate static control signal independent of the reset tree. It must be asserted alongside reset isolation for a complete and safe harvest. The 12-bit vector covers all 12 Tensix tiles in bit order `[x + 4*y]`.

---

*Document generated from RTL analysis of `/secure_data_from_tt/20260221/rtl/trinity.sv`, `trinity_pkg.sv`, `trinity_noc2axi_nw_opt.sv`, `trinity_noc2axi_n_opt.sv`, `trinity_noc2axi_ne_opt.sv`, `trinity_router.sv`, and `filelist.f`. N1B0 corrections applied 2026-03-19.*
