...
**Document:** router_decode_HDD_v0.2.md
...
```
AXI address [51:0] layout:
 ┌──────────┬──────────┬────────────────────────────┐
 │ [51:46]  │ [45:40]  │ [39:0]                     │
 │  Y coord │  X coord │  Local address (40-bit)    │
 │  (6-bit) │  (6-bit) │                            │
 └──────────┴──────────┴────────────────────────────┘
```

- Bits [63:52] of the 64-bit NoC address are dropped (upper 12 bits unused)
- The AXI interface is 56-bit wide (`NPU_IN_ADDR_W = 56` in `trinity.sv`)
- The gasket extracts X/Y coordinates directly from these fixed bit positions
...
`tensix_id` mapping (SW convention): `tensix_id = y × SizeX + x` (row-major) or `tensix_id = x × SizeY + y` (column-major, matches `EndpointIndex` in `trinity_pkg.sv`).
