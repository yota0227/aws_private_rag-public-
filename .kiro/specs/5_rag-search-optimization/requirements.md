...
- **Metadata_File**: S3에 문서와 함께 저장되는 `.metadata.json` 파일로, Bedrock KB가 문서 필터링에 사용하는 속성(팀, 카테고리, 문서 유형, 버전, 소스 시스템 등)을 포함
...
2. THE Lambda_Handler SHALL Metadata_File에 다음 필드를 포함한다: `metadataAttributes` 객체 내에 `team`(문자열), `category`(문자열), `document_type`(문자열), `upload_date`(ISO 8601 형식 문자열), `version`(문자열), `source_system`(문자열).
3. WHEN 업로드 요청에 `team` 또는 `category` 값이 포함되지 않으면, THE Lambda_Handler SHALL 해당 필드를 빈 문자열(`""`)로 설정하여 Metadata_File을 생성한다.
3a. THE Lambda_Handler SHALL `version` 필드를 업로드 요청에서 제공된 값으로 설정하며, 값이 제공되지 않으면 기본값 `"1.0"`을 사용한다. 파일명에 버전 패턴(예: `_v0.1`, `_v2.3`)이 포함된 경우 해당 버전을 자동 추출하여 사용할 수 있다.
3b. THE Lambda_Handler SHALL `source_system` 필드를 업로드 요청에서 제공된 값으로 설정하며, 값이 제공되지 않으면 기본값 `"manual_upload"`를 사용한다. 허용되는 값의 예시: `"manual_upload"`, `"codebeamer"`, `"cli"`.
...
4a. THE Lambda_Handler SHALL `filter` 객체 내 `source_system` 필드가 존재하면 `{"equals": {"key": "source_system", "value": "<source_system값>"}}` 형식의 Bedrock_KB 필터를 생성하며, 다른 필터 조건과 함께 `{"andAll": [...]}` 연산자로 결합한다.
...
2. THE Lambda_Handler SHALL `citations` 배열의 각 항목에 `text`(인용 텍스트), `references`(배열) 필드를 포함하며, 각 `references` 항목에는 `uri`(S3 URI), `score`(관련도 점수, 숫자 또는 null) 필드를 포함한다. 각 `references` 항목에 해당 문서의 `version`과 `source_system` 메타데이터가 존재하면 해당 값을 포함한다.
...
1. THE MCP_Bridge SHALL `rag_query` 도구의 입력 스키마에 `team`(선택적 문자열), `category`(선택적 문자열), `source_system`(선택적 문자열) 파라미터를 추가한다.
2. WHEN `team`, `category`, 또는 `source_system` 파라미터 중 하나라도 제공되면, THE MCP_Bridge SHALL API Gateway 호출 시 요청 본문의 `filter` 객체에 해당 값을 포함하여 전달한다.
3. WHEN `team`, `category`, `source_system` 파라미터가 모두 제공되지 않으면, THE MCP_Bridge SHALL 요청 본문에 `filter` 객체를 포함하지 않는다.
...
2. THE Lambda_Handler SHALL 식별된 각 문서에 대해 S3 키 경로에서 `team`과 `category`를 파싱하여(`documents/{team}/{category}/{filename}` 형식) Metadata_File을 생성하며, 경로에서 파싱할 수 없는 경우 빈 문자열(`""`)을 사용한다. `document_type`은 파일 확장자 기반으로 설정한다. `version`은 기본값 `"1.0"`, `source_system`은 기본값 `"manual_upload"`로 설정한다.
