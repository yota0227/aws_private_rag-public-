...
3. 메타데이터 기반 필터링: team/category/source_system 기반 검색 범위 제한
4. 검색 응답 구조 개선: score, metadata 필드 추가로 결과 신뢰성 판단 지원
5. MCP Bridge 필터 전달: Obot 채팅에서 팀/카테고리/소스 시스템 필터 지정 가능
...
sequenceDiagram
    ...
    User->>MCP: rag_query(query, team?, category?, source_system?)
...
sequenceDiagram
    ...
    User->>Lambda: confirm_upload(s3_key, team, category, version?, source_system?)
...
- 응답에서 `retrievedReferences`의 `version`, `source_system` 메타데이터가 존재하면 `references[]`에 포함
...
def handle_query(event):
    ...
    try:
        ...
        # 응답 구성 (score + version/source_system 포함)
        citations = []
        for c in resp.get('citations', []):
            ...
            for r in c.get('retrievedReferences', []):
                ...
                ref_entry = {'uri': uri, 'score': score}
                # version/source_system 메타데이터가 존재하면 포함
                ref_metadata = r.get('metadata', {})
                if ref_metadata.get('version'):
                    ref_entry['version'] = ref_metadata['version']
                if ref_metadata.get('source_system'):
                    ref_entry['source_system'] = ref_metadata['source_system']
...
기존 `confirm_upload()`에 메타데이터 생성 + fire-and-forget KB Sync 로직 추가. Eventual Consistency 모델을 수용하여 CRR 복제 대기 없이 즉시 KB Sync를 트리거한다. 요청 body에서 `version`(기본값 `"1.0"`)과 `source_system`(기본값 `"manual_upload"`)을 읽어 `create_metadata_file()`에 전달한다:
...
def confirm_upload(event):
    ...
    # 메타데이터 파일 생성
    metadata_key = create_metadata_file(s3_key, team, category, version, source_system)
    ...
    while True:
        ...
        for obj in resp.get('Contents', []):
            ...
            try:
                ...
                try:
                    create_metadata_file(key, team, category, version='1.0', source_system='manual_upload')
                    processed_count += 1
                except Exception as e:
...
`rag_query` 도구의 입력 스키마에 `team`, `category`, `source_system` 파라미터 추가:
...
mcp.tool(
  ...
  "BOS-AI RAG 지식 베이스에 질의합니다. ...",
  {
    ...
    source_system: z.string().optional().describe("소스 시스템 필터 (예: manual_upload, codebeamer, cli). 특정 출처 문서만 검색")
  },
  async (args) => {
    const body = { query: args.query };
    if (args.team || args.category || args.source_system) {
      body.filter = {};
      if (args.team) body.filter.team = args.team;
      if (args.category) body.filter.category = args.category;
      if (args.source_system) body.filter.source_system = args.source_system;
    }
...
| `version` | string | 문서 버전. 기본값 `"1.0"` | `"1.0"`, `"2.3"` |
| `source_system` | string | 문서 출처 시스템. 기본값 `"manual_upload"` | `"manual_upload"`, `"codebeamer"`, `"cli"` |
...
단일 조건 (team만 또는 category만 또는 source_system만):
```json
{"equals": {"key": "team", "value": "soc"}}
```

복합 조건 (team + category + source_system 등 복수 조건):
```json
{
  "andAll": [
    {"equals": {"key": "team", "value": "soc"}},
    {"equals": {"key": "category", "value": "code"}},
    {"equals": {"key": "source_system", "value": "codebeamer"}}
  ]
}
```
...
*For any* S3 키와 임의의 team/category/version/source_system 값에 대해, `create_metadata_file()`이 생성하는 메타데이터는 `metadataAttributes` 객체를 포함하고, 그 안에 `team`(문자열), `category`(문자열), `document_type`(문자열), `upload_date`(ISO 8601 형식 문자열), `version`(문자열), `source_system`(문자열) 필드가 모두 존재해야 한다. team 또는 category가 None이거나 빈 값이면 빈 문자열(`""`)로 설정되어야 하며, version이 None이거나 빈 값이면 `"1.0"`, source_system이 None이거나 빈 값이면 `"manual_upload"`로 설정되어야 한다. `document_type`은 파일 확장자 매핑(`.pdf`→`"pdf"`, `.txt`→`"text"`, `.md`→`"markdown"`, 그 외→`"other"`)을 정확히 따라야 한다. 메타데이터 키는 원본 S3 키 + `.metadata.json`이어야 한다.

**Validates: Requirements 3.1, 3.2, 3.3, 3.3a, 3.3b, 3.4, 3.5**

### Property 4: Bedrock 필터 구문 변환

*For any* filter 객체에 대해, `build_bedrock_filter()`는 다음 규칙을 따라야 한다: 단일 필터 키(team, category, source_system 중 하나)만 있으면 `{"equals": {"key": "<키>", "value": "<값>"}}`, 복수 필터 키가 있으면 `{"andAll": [...]}` 연산자로 결합, 모든 키가 없거나 빈 값이면 `None`을 반환해야 한다.

**Validates: Requirements 4.1, 4.2, 4.3, 4.4, 4.4a, 4.5**

### Property 5: 질의 응답 구조 완전성

*For any* 성공적인 Bedrock `retrieve_and_generate` 응답에 대해, Lambda의 질의 응답은 `answer`(문자열), `citations`(배열), `metadata`(객체) 필드를 포함해야 한다. 각 citation은 `text`(문자열)와 `references`(배열)를 포함하고, 각 reference는 `uri`(문자열)와 `score`(숫자 또는 null)를 포함해야 한다. Bedrock 응답 메타데이터에 `version` 또는 `source_system`이 존재하면 해당 reference에 포함되어야 한다. `metadata`는 `search_type`, `results_count`, `query_length` 필드를 포함해야 한다.

**Validates: Requirements 5.1, 5.2, 5.3, 5.4**

### Property 6: MCP Bridge 필터 요청 구성

*For any* team, category, source_system 파라미터 조합에 대해, MCP Bridge가 구성하는 API 요청 본문은 다음을 만족해야 한다: team, category, source_system 중 하나라도 제공되면 `filter` 객체에 해당 값이 포함되고, 모두 미제공이면 `filter` 객체가 요청 본문에 포함되지 않아야 한다.
...
| MCP Bridge `rag_query` 스키마에 team/category/source_system 존재 | 요구사항 6.1 |
