...
- [x] 1. 순수 로직 함수 구현 및 속성 기반 테스트
  ...
  - [x]* 1.2 Pipeline_ID 파싱 속성 테스트 작성
  ...
  - [x]* 1.12 Claim 스키마 및 토큰 분할 속성 테스트 작성
  ...
  - [x]* 1.14 검색 쿼리 및 Variant Delta 속성 테스트 작성
...
- [x] 3. Terraform 인프라 구성
  ...
  - [x]* 3.6 Terraform 리소스 태깅 속성 테스트 작성
...
- [x] 7. LLM 통합 — Claim Generator 및 HDD Section Generator
  ...
  - [x]* 7.3 HDD 섹션 완전성 및 참조 무결성 속성 테스트 작성
...
- [x] 9. Variant Delta 분석 및 검색 통합
  ...
  - [x]* 9.3 Pipeline_ID 격리 속성 테스트 작성
...
- [x] 11. 서브시스템 심화 분석 — 패키지 파라미터, EDC, NoC, Overlay
  ...
  - [x]* 11.12 Overlay 구조 분석 속성 테스트 작성
...
- [x] 14. HDD Generator 심화 분석 결과 통합
  ...
  - [x]* 14.5 단위 테스트 — 심화 분석 데이터가 프롬프트에 포함되는지 검증
...
- [x] 15. chip_config 분석 타임아웃 해결
  ...
  - [x]* 15.4 단위 테스트 — *_pkg.sv 파일 필터링 정확성 검증
...
- [x] 16. SRAM Inventory Extractor 구현
  ...
  - [x]* 16.5 단위 테스트 — 메모리 패턴 매칭, 파라미터 추출, unknown 기본값 처리
...
- [x] 22. v3 R2 이슈 수정 — MCP Bridge search_rtl 파라미터 패스스루 (이슈 #1, #5)
  - [x] 22.1 온프레미스 MCP Bridge server.js의 search_rtl 도구 파라미터 스키마 수정
  ...
  - [x] 22.2 MCP Bridge search_rtl 파라미터 전달 검증 테스트 가이드 작성
...
- [x] 23. v3 R2 이슈 수정 — 배포 및 E2E 검증
  ...
  - [x] 23.6 온프레미스 MCP Bridge 업데이트 및 Obot E2E 검증
...
- [x] 24. Final Checkpoint - v3 R2 이슈 수정 전체 검증
...
## v5: 검색 정밀도 개선 (Grounding 46% → 65% 목표)

### v5 배경

v4.1에서 커버리지 87%를 달성했으나 KB Grounding은 46%에 불과. LLM inference 의존도가 높아 신뢰성이 부족하다. v5의 목표는 검색 경로를 최적화하여 KB에서 직접 확인 가능한 데이터 비율을 높이는 것.

### v5 발견된 문제점

| 문제 | 원인 | 영향 |
|------|------|------|
| analysis_type 필터 미전달 | `search_archive()` → `_search_rtl_index()`에서 analysis_type 파라미터 누락 | claim/hdd_section 필터 검색 불가 |
...
- [ ] 27. v5 E2E 검증 — Grounding 비율 측정
  - [ ] 27.1 Obot에서 v5 프롬프트 세트 실행
    - 프롬프트 1번 (칩 전체, Grounding 없음) → `v5/v5a_chip_no_grounding.md`
    - 프롬프트 9번 (통합, Grounding 있음) → `v5/v5b_chip_grounded.md`
    - 프롬프트 2번 (EDC) → `v5/v5_edc.md`
    - 프롬프트 4번 (NoC) → `v5/v5_noc.md`
    - 프롬프트 3번 (Overlay) → `v5/v5_overlay.md`

  - [ ] 27.2 v4.1 대비 Grounding 비율 비교
    - v4.1b grounded: 46% → v5b 목표: 65%
    - analysis_type=claim 필터로 claim 검색 결과 확인
    - analysis_type=hdd_section 필터로 HDD 검색 결과 확인
    - [NOT IN KB] 항목 수 감소 확인

  - [ ] 27.3 prompt.md에 RAG v4 버전 정의 추가
    - RAG v4: 검색 정밀도 개선 (analysis_type 패스스루, 가중치 조정)
    - 저장 위치: `v5/`

- [ ] 28. v5 Final Checkpoint
  - analysis_type 필터가 MCP Bridge → API Gateway → RTL Lambda 전체 경로에서 동작하는지 확인
  - claim/hdd_section 검색 시 해당 타입 문서만 반환되는지 확인
  - Grounding 비율 46% → 65% 달성 여부 확인
  - 기존 파이프라인(Task 1~24)과의 호환성 확인
