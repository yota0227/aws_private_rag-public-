...
- [ ] 14. HDD Generator 심화 분석 결과 통합
  - [ ] 14.1 hdd_generator.py의 generate_hdd_section() 함수에서 심화 분석 결과 조회 로직 추가
  ...
  - [ ] 14.2 analysis_handler.py의 handle_hdd_generation() 함수에서 심화 분석 결과 5종 조회 및 전달
  ...
  - [ ] 14.3 hdd_generator.py의 _build_hdd_prompt() 함수 확장 — 토픽별 심화 데이터를 LLM 프롬프트에 포함
  ...
  - [ ] 14.4 심화 분석 결과 조회 실패 시 기본 HDD 생성으로 폴백하는 에러 처리
...
- [ ] 15. chip_config 분석 타임아웃 해결
  - [ ] 15.1 analysis_handler.py의 handle_chip_config() 함수에서 *_pkg.sv 파일만 선별적으로 S3에서 읽는 로직 구현
  ...
  - [ ] 15.2 OpenSearch module_parse 문서의 file_path에서 *_pkg.sv 패턴 필터링으로 S3 ListObjects 호출 최소화
  ...
  - [ ] 15.3 300초 타임아웃 임박 시 미처리 토픽을 별도 Lambda로 분할 실행하는 로직 구현
...
- [ ] 16. SRAM Inventory Extractor 구현
  - [ ] 16.1 sram_inventory.py 모듈 생성 — 메모리 패턴 매칭 함수
  ...
  - [ ] 16.2 메모리 인스턴스 파라미터(depth, width, ECC) 추출 함수 구현
  ...
  - [ ] 16.3 analysis_handler.py에 handle_sram_inventory() 핸들러 추가 + _STAGE_HANDLERS 등록
  ...
  - [ ] 16.4 OpenSearch에 analysis_type=sram_inventory 문서 인덱싱
...
- [ ] 17. Lambda 배포 및 E2E 검증
  - [ ] 17.1 수정된 hdd_generator.py, analysis_handler.py, 신규 sram_inventory.py를 Lambda 배포 패키지에 포함
  ...
  - [ ] 17.2 chip_config 분석 재실행 — *_pkg.sv 선별 읽기로 타임아웃 해결 확인
  ...
  - [ ] 17.5 Obot에서 search_rtl로 개선된 HDD 검색 검증
...
- [ ] 18. Final Checkpoint - 신규 기능 전체 검증
