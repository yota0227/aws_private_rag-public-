...
- [ ] 19. v3 R2 이슈 수정 — 포트 비트폭 추출 (이슈 #4)
  - [ ] 19.1 handler.py의 parse_rtl_to_ast() 포트 정규식 확장 — 비트폭 캡처 그룹 추가
    - `environments/app-layer/bedrock-rag/rtl_parser_src/handler.py` 수정
...
- [ ] 20. v3 R2 이슈 수정 — 토픽 분류 확장 (이슈 #3)
  - [ ] 20.1 topic_classifier.py의 TOPIC_RULES에 Power, Memory 토픽 추가 및 NIU 접두사 확장
...
- [ ] 21. v3 R2 이슈 수정 — Claim 귀속 정확성 개선 (이슈 #2)
  - [ ] 21.1 claim_generator.py에 레지스터 래퍼 필터링 및 다양성 검증 함수 추가
  ...
  - [ ] 21.2 analysis_handler.py의 handle_hdd_generation()에 토픽-모듈 교차 검증 추가
...
- [ ] 23. v3 R2 이슈 수정 — 배포 및 E2E 검증
  - [ ] 23.1 수정된 handler.py, topic_classifier.py, claim_generator.py, analysis_handler.py Lambda 배포
