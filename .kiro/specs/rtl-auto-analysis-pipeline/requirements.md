...
- **SRAM_Inventory_Extractor**: 계층 트리에서 메모리 인스턴스(SRAM, register file, ROM)를 식별하여 인벤토리를 생성하는 분석 컴포넌트
- **MCP_Bridge**: 온프레미스 환경에서 Obot과 RAG API Gateway를 연결하는 Node.js MCP SSE 브릿지 서버 (`mcp-bridge/server.js`). search_rtl 도구를 통해 RTL OpenSearch 인덱스 검색을 지원한다
...
7. WHEN HDD 섹션을 생성할 때, THE HDD_Section_Generator SHALL OpenSearch에서 chip_config, edc_topology, noc_protocol, overlay_deep analysis_type의 분석 결과를 조회하여 HDD 콘텐츠에 반영한다
8. WHEN 칩 전체 HDD를 생성할 때, THE HDD_Section_Generator SHALL chip_config 분석 결과에서 Package Constants 섹션(SizeX/SizeY, NumTensix, tile_t enum, Endpoint Index Table)을 포함한다
9. WHEN EDC HDD를 생성할 때, THE HDD_Section_Generator SHALL edc_topology 분석 결과에서 링 토폴로지 다이어그램, 시리얼 버스 인터페이스 사양, 하베스트 바이패스 흐름도를 포함한다
10. WHEN NoC HDD를 생성할 때, THE HDD_Section_Generator SHALL noc_protocol 분석 결과에서 라우팅 알고리즘 3종 비교 테이블, flit 헤더 구조, AXI 주소 가스켓 매핑을 포함한다
11. WHEN Overlay HDD를 생성할 때, THE HDD_Section_Generator SHALL overlay_deep 분석 결과에서 CPU 클러스터 파라미터, L1 캐시 구성 테이블, APB 슬레이브 레지스터 맵을 포함한다
...
6. WHEN chip_config 분석을 실행할 때, THE Analysis_Orchestrator SHALL S3에서 `*_pkg.sv` 파일만 선별적으로 읽어 처리하고, 전체 RTL 파일(9,465개)을 순회하지 않는다
7. IF chip_config 분석이 단일 Lambda 실행에서 300초 타임아웃을 초과할 위험이 있으면, THEN THE Analysis_Orchestrator SHALL 토픽별(noc_pkg, edc_pkg, overlay_pkg 등)로 분할하여 개별 Lambda 호출로 실행한다
8. THE RTL_Parser_Lambda SHALL chip_config 분석 시 S3 ListObjects 호출을 최소화하기 위해 파일명 패턴 필터(`*_pkg.sv`)를 S3 Prefix 또는 사전 구축된 파일 목록 기반으로 적용한다
...
### 요구사항 16: SRAM Inventory 자동 추출

**사용자 스토리:** 엔지니어로서, RTL 코드에서 메모리 인스턴스(SRAM, register file, ROM)가 자동으로 식별되어 인벤토리로 정리되기를 원한다. 이를 통해 칩 전체의 메모리 자원 현황을 수동으로 집계하지 않아도 된다.

#### 수용 기준

1. WHEN 계층 트리가 구축되면, THE Analysis_Orchestrator SHALL 각 모듈의 인스턴스 목록에서 메모리 인스턴스(SRAM, register file, ROM)를 패턴 매칭으로 식별한다
2. THE Analysis_Orchestrator SHALL 식별된 메모리 인스턴스에 대해 다음 정보를 추출한다: 인스턴스명, 메모리 타입(SRAM/RF/ROM), 소속 모듈, 계층 경로, 파라미터(depth, width, ECC 유무)
3. THE Analysis_Orchestrator SHALL SRAM Inventory 결과를 OpenSearch에 `analysis_type=sram_inventory` 문서로 인덱싱한다
4. WHEN HDD_Section_Generator가 칩 전체 HDD를 생성할 때, THE HDD_Section_Generator SHALL SRAM Inventory 섹션을 포함하여 메모리 타입별 수량, 총 용량, 소속 서브시스템별 분포를 테이블로 표시한다
5. THE Analysis_Orchestrator SHALL 메모리 인스턴스 식별 시 다음 패턴을 사용한다: 모듈명에 `sram`, `ram`, `rf_`, `reg_file`, `rom`, `memory`, `mem_` 키워드가 포함된 인스턴스
6. IF 메모리 인스턴스의 depth 또는 width 파라미터를 RTL에서 추출할 수 없으면, THEN THE Analysis_Orchestrator SHALL 해당 필드를 "unknown"으로 표시하고 경고 로그를 기록한다

### 요구사항 17: MCP Bridge 파라미터 패스스루 (v3 R2 이슈 #1, #5)

**사용자 스토리:** 엔지니어로서, MCP Bridge의 search_rtl 도구가 max_results, analysis_type 등 모든 파라미터를 Lambda API에 정확히 전달하기를 원한다. 이를 통해 검색 결과 건수와 필터링이 의도대로 동작할 수 있다.

#### 수용 기준

1. WHEN MCP_Bridge의 search_rtl 도구가 호출되면, THE MCP_Bridge SHALL max_results 파라미터를 Lambda API 요청 본문에 그대로 전달한다
2. WHEN MCP_Bridge의 search_rtl 도구가 호출되면, THE MCP_Bridge SHALL analysis_type 파라미터를 Lambda API 요청 본문에 그대로 전달한다
3. WHEN MCP_Bridge의 search_rtl 도구가 호출되면, THE MCP_Bridge SHALL pipeline_id, topic 파라미터를 Lambda API 요청 본문에 그대로 전달한다
4. THE MCP_Bridge의 search_rtl 도구 SHALL max_results의 기본값을 20으로 설정한다
5. IF MCP_Bridge의 search_rtl 도구에 선택적 파라미터가 제공되지 않으면, THEN THE MCP_Bridge SHALL 해당 파라미터를 Lambda API 요청에서 생략한다

### 요구사항 18: Claim 귀속 정확성 및 토픽-모듈 매핑 검증 (v3 R2 이슈 #2)

**사용자 스토리:** 엔지니어로서, Claim이 해당 토픽의 실제 대표 모듈에 올바르게 귀속되기를 원한다. 이를 통해 HDD 생성 시 단일 모듈 편중 없이 토픽별로 다양한 모듈의 설계 지식이 포함될 수 있다.

#### 수용 기준

1. WHEN Claim_Generator가 토픽별 claim을 생성할 때, THE Claim_Generator SHALL 해당 토픽의 계층 트리에서 데이터패스 모듈(레지스터 래퍼 `_reg_inner`, `_wrap` 패턴이 아닌 모듈)을 우선적으로 claim 대상에 포함한다
2. WHEN HDD_Section_Generator가 토픽별 HDD를 생성할 때, THE HDD_Section_Generator SHALL claim의 module_name이 해당 토픽의 실제 모듈 계층에 속하는지 검증하고, 불일치하는 claim을 제외한다
3. THE Claim_Generator SHALL 토픽별 claim 생성 시 최소 3개 이상의 서로 다른 모듈에서 claim을 생성한다
4. IF 토픽의 모든 claim이 단일 모듈에 편중되면(전체 claim의 80% 이상이 동일 모듈), THEN THE Claim_Generator SHALL 경고 로그를 기록하고 해당 토픽의 다른 모듈에서 추가 claim 생성을 시도한다
5. THE Claim_Generator SHALL claim 생성 시 classify_topic() 결과와 hierarchy 위치를 교차 검증하여, 토픽 분류와 계층 위치가 일치하는 모듈에만 claim을 귀속한다

### 요구사항 19: 토픽 분류 확장 — NIU, Power, Memory 토픽 추가 (v3 R2 이슈 #3)

**사용자 스토리:** 엔지니어로서, NIU, Power, Memory 관련 모듈이 올바른 토픽으로 분류되기를 원한다. 이를 통해 NIU가 NoC에 묻히지 않고 독립 토픽으로 검색되며, 전력 관리 및 메모리 관련 모듈도 별도 토픽으로 분류될 수 있다.

#### 수용 기준

1. THE Topic_Classifier SHALL NIU 토픽을 독립적으로 분류한다: 파일 경로 패턴 `*/niu/*`와 모듈명 접두사 `tt_niu_`, `niu_`, `tt_noc2axi_*`를 NIU 토픽에 매핑한다
2. THE Topic_Classifier SHALL Power 토픽을 신규 추가한다: 파일 경로 패턴 `*/power/*`, `*/prtn/*`와 모듈명 접두사 `tt_prtn_`, `prtn_`, `iso_en_*`를 Power 토픽에 매핑한다
3. THE Topic_Classifier SHALL Memory 토픽을 신규 추가한다: 파일 경로 패턴 `*/memory/*`, `*/sram/*`와 모듈명 접두사 `sfr_*`, `tt_mem_`, `mem_`를 Memory 토픽에 매핑한다
4. THE Topic_Classifier SHALL 최소 다음 토픽 카테고리를 지원한다: NoC, FPU, SFPU, TDMA, Overlay, EDC, Dispatch, L1_Cache, Clock_Reset, DFX, NIU, SMN, Power, Memory
5. WHEN `tt_noc2axi_*` 패턴의 모듈이 분류될 때, THE Topic_Classifier SHALL 해당 모듈을 NIU 토픽으로 분류한다 (NoC 토픽이 아닌)

### 요구사항 20: 포트 비트폭 및 배열 인덱스 추출 (v3 R2 이슈 #4)

**사용자 스토리:** 엔지니어로서, RTL 파서가 포트 선언에서 비트폭과 배열 인덱스를 추출하기를 원한다. 이를 통해 `i_ai_clk`이 1비트인지 `[SizeX-1:0]` 배열인지 정확히 파악할 수 있다.

#### 수용 기준

1. WHEN RTL_Parser_Lambda가 포트 선언을 파싱할 때, THE RTL_Parser_Lambda SHALL `[MSB:LSB]` 형식의 비트폭 정보를 추출하여 port_list 결과에 포함한다
2. WHEN 포트 선언에 파라미터 기반 비트폭(예: `[SizeX-1:0]`)이 포함되면, THE RTL_Parser_Lambda SHALL 해당 표현식을 문자열 그대로 추출한다
3. THE RTL_Parser_Lambda SHALL 포트 선언의 비트폭 정보를 `direction [bit_width] port_name` 형식으로 port_list에 저장한다 (예: `input [SizeX-1:0] i_ai_clk`)
4. WHEN 포트 선언에 비트폭이 명시되지 않으면, THE RTL_Parser_Lambda SHALL 해당 포트를 1비트로 간주하고 비트폭 정보를 생략한다
5. THE RTL_Parser_Lambda SHALL 다차원 배열 포트(예: `input [3:0][7:0] data`)의 모든 차원 정보를 추출한다

### 요구사항 21: MCP Bridge search_rtl 도구 파라미터 완전성 (v3 R2 이슈 #5)

**사용자 스토리:** 인프라 운영자로서, 온프레미스 MCP Bridge의 search_rtl 도구가 Lambda API의 모든 검색 파라미터를 지원하기를 원한다. 이를 통해 Obot에서 RTL 검색 시 max_results, analysis_type, pipeline_id, topic 필터를 모두 활용할 수 있다.

#### 수용 기준

1. THE MCP_Bridge의 search_rtl 도구 SHALL 다음 파라미터를 Zod 스키마로 정의한다: query(필수), max_results(선택, 기본값 20), analysis_type(선택), pipeline_id(선택), topic(선택)
2. WHEN search_rtl 도구가 호출되면, THE MCP_Bridge SHALL Lambda를 동기 invoke하여 `action: "search"` 요청을 전달하고, 응답의 results 배열을 MCP 텍스트 콘텐츠로 반환한다
3. THE MCP_Bridge SHALL search_rtl 도구의 응답에 total_hits, 각 결과의 module_name, analysis_type, topic, score 필드를 포함한다
4. IF Lambda invoke가 실패하거나 타임아웃(30초)이 발생하면, THEN THE MCP_Bridge SHALL 에러 메시지를 MCP isError 응답으로 반환한다

### 요구사항 22: API Gateway Lambda 검색 경로 파라미터 패스스루 (v5)

**사용자 스토리:** 엔지니어로서, MCP Bridge에서 전달한 analysis_type, pipeline_id, topic 파라미터가 API Gateway Lambda(`search_archive`)를 거쳐 RTL Parser Lambda(`_search_rtl`)까지 정확히 전달되기를 원한다. 이를 통해 claim이나 hdd_section만 필터링하여 검색할 수 있고, Grounding 비율을 높일 수 있다.

#### 수용 기준

1. WHEN MCP_Bridge가 search_rtl 도구를 호출하면, THE API Gateway Lambda의 `search_archive()` 함수 SHALL body에서 `analysis_type` 파라미터를 읽어 `_search_rtl_index()`에 전달한다
2. THE `_search_rtl_index()` 함수 SHALL `analysis_type` 파라미터를 RTL Parser Lambda invoke payload에 조건부 포함한다 (값이 있을 때만)
3. THE `_search_rtl_index()` 함수의 응답 SHALL 각 결과에 analysis_type, topic, claim_text, claim_type, hdd_content, hdd_section_title 필드를 포함한다
4. WHEN analysis_type 필터가 지정되면, THE 검색 결과 SHALL 해당 analysis_type의 문서만 포함한다 (예: `analysis_type=claim` → claim 문서만)
5. THE `_search_rtl_index()` 함수 SHALL analysis_type별로 응답 텍스트 포맷을 분기한다 (claim → claim_text 표시, hdd_section → hdd_content 표시, module_parse → port_list/instance_list 표시)

### 요구사항 23: 검색 가중치 최적화 — Claim/HDD 우선 검색 (v5)

**사용자 스토리:** 엔지니어로서, RTL 검색 시 module_parse(10,000건)보다 claim(99건)과 hdd_section(12건)이 우선적으로 반환되기를 원한다. 이를 통해 검색 결과에서 설계 지식(claim, HDD)을 먼저 확인하고, 필요 시 원본 파싱 데이터(module_parse)를 참조할 수 있다.

#### 수용 기준

1. WHEN analysis_type 필터가 지정되지 않은 일반 검색 시, THE RTL_Parser_Lambda의 `_search_rtl()` 함수 SHALL claim_text와 hdd_content 필드에 boost 가중치(2.0 이상)를 적용하여 claim/hdd_section 문서가 module_parse보다 상위에 노출되도록 한다
2. THE `_search_rtl()` 함수 SHALL analysis_type 미지정 시 should 절에 `analysis_type=claim` (boost 3.0)과 `analysis_type=hdd_section` (boost 2.0)을 추가하여 해당 타입 문서를 우선 반환한다
3. THE `_search_rtl()` 함수 SHALL module_parse 문서의 module_name wildcard 검색 boost를 1.5로 설정하고, analysis_type 미지정 시 should 절에 `analysis_type=module_parse` (boost 1.0)을 추가하여 module_parse 데이터도 검색 결과에 포함되도록 한다
4. WHEN analysis_type 필터가 지정된 경우, THE `_search_rtl()` 함수 SHALL 가중치 조정 없이 해당 타입 문서만 정확히 필터링한다

### 요구사항 24: Package Parser — localparam/enum/struct 추출 (v6)

**사용자 스토리:** 엔지니어로서, `*_pkg.sv` 파일의 localparam, typedef enum, typedef struct 정의가 KB에 구조화된 claim으로 인덱싱되기를 원한다.

#### 수용 기준

1. THE `package_extractor.py`의 `is_package_file()` SHALL `*_pkg.sv` 또는 `*_pkg.v` 파일을 감지한다
2. THE `extract_package_constants()` SHALL localparam (int/integer/logic/bit/string 등), typedef enum, typedef struct를 추출한다
3. THE 추출 결과 SHALL `analysis_type=claim`, `topic=PackageConfig`로 인덱싱된다
4. THE localparam 추출 SHALL 모든 localparam에 개별 claim을 생성한다 (표현식 값 포함)
5. THE `_process_rtl_file()` SHALL package 파일 감지 시 `extract_package_constants()`를 호출한다

### 요구사항 25: Port Classifier — 카테고리별 포트 claim 생성 (v7)

**사용자 스토리:** 엔지니어로서, 대형 모듈의 포트가 기능 카테고리별 claim으로 분류되기를 원한다. LLM attention 문제 완화.

#### 수용 기준

1. THE `port_classifier.py`의 `classify_ports()` SHALL 포트 10개 이상 모듈에 카테고리별 claim 생성
2. THE 카테고리 SHALL PRTN_Power, EDC_APB, AXI_Interface, APB_Register, DM_Clock_Reset, AI_Clock_Reset, NoC_Clock_Reset, Tensix_Reset, SFR_Memory_Config 9개 포함
3. THE claim SHALL `analysis_type=claim`, `topic=TopLevelPorts`로 인덱싱
4. THE 함수 SHALL 카테고리 요약 claim을 추가 생성
5. THE `_process_rtl_file()` SHALL 포트 10개 이상 모듈에 `classify_ports()` 호출

### 요구사항 26: MCP Bridge 응답 길이 확대 (v7)

**사용자 스토리:** 엔지니어로서, MCP Bridge 응답에서 port_list 등이 충분한 길이로 반환되기를 원한다.

#### 수용 기준

1. THE `server.js` search_rtl 응답 SHALL port_list 최대 800자 (기존 200)
2. THE instance_list SHALL 최대 800자 (기존 200)
3. THE hdd_content SHALL 최대 800자 (기존 300)
4. THE claim_text SHALL 최대 500자 (기존 300)
5. THE 최악 케이스(20건 × 800자) SHALL LLM 컨텍스트 20% 이내
