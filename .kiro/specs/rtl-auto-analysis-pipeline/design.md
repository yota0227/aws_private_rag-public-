...
## v6/v7 파이프라인 확장 (2026-04-28)

### 20. Package Parser (v6 — `package_extractor.py`)

`*_pkg.sv` 파일의 localparam/enum/struct를 claim으로 추출. 기존 module_parse는 package 상수를 놓침.

```
S3 Event → _process_rtl_file() → is_package_file()? → extract_package_constants()
  → [localparam + enum + struct claims] → _index_to_opensearch(topic=PackageConfig)
```

추출: localparam(int 포함), typedef enum, typedef struct, parameter.
파일: `rtl_parser_src/package_extractor.py`

### 21. Port Classifier (v7 — `port_classifier.py`)

대형 모듈 포트를 9개 카테고리별 claim으로 분류. LLM attention 문제 완화.

```
_process_rtl_file() → port_list >= 10? → classify_ports()
  → [PRTN_Power, EDC_APB, AXI, APB, DM_Clock, AI_Clock, NoC_Clock, Tensix, SFR]
  → _index_to_opensearch(topic=TopLevelPorts)
```

파일: `rtl_parser_src/port_classifier.py`

### 22. MCP Bridge 응답 확대 (v7)

port_list 200→800, instance_list 200→800, hdd_content 300→800, claim_text 300→500.
파일: `mcp-bridge/server.js`

### 23. _index_to_opensearch claim 필드 (v6)

doc에 claim_text/claim_type/claim_id/topic 추가. isinstance 체크로 list/string 방어.
...
|------|------|----------|----------|
| v1 | 4/17 | 기본 파싱 | 12% |
| v5.1 | 4/27 | module_parse 가중치 리밸런싱 | 13% |
| v6c | 4/28 | Package parser (13개 상수 + tile_t + EP) | 45% |
| v7 | 4/28 | Port classifier + MCP 800자 + TDMA 3 sub-module | 50%+ |
