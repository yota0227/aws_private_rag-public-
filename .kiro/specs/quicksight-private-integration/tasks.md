...
- [ ] 1. Global IAM 레이어 — Quick IAM 역할 및 Identity Center 그룹 생성
  - [ ] 1.1 `environments/global/iam/`에 Quick IAM 역할 3개 추가
  ...
  - [ ] 1.4 `environments/global/iam/`에 IAM Identity Center 그룹 3개 추가
...
- [ ] 2. 체크포인트 — Global IAM 레이어 검증
  - `terraform validate && tflint` 실행하여 IAM 리소스 구성 확인
  - 모든 테스트 통과 여부 확인. 문제가 있으면 사용자에게 질문하세요.

- [ ] 3. Network Layer — Quick VPC Endpoint, Security Group, Route53 DNS
  - [ ] 3.1 `environments/network-layer/quicksight-security-groups.tf` 생성
  ...
  - [ ] 3.4 `environments/network-layer/quicksight-vpc-endpoints.tf` 생성
  ...
  - [ ] 3.6 `environments/network-layer/quicksight-dns.tf` 생성
  ...
  - [ ] 3.7 Quick VPC Connection ENI 서브넷 라우팅 테이블 확인 및 보완
...
- [ ] 4. 체크포인트 — Network Layer 검증
  - `terraform validate && tflint` 실행하여 VPC Endpoint, SG, DNS 리소스 구성 확인
  - 모든 테스트 통과 여부 확인. 문제가 있으면 사용자에게 질문하세요.

- [ ] 5. App Layer — Quick 계정, VPC Connection, S3, Lambda, 모니터링
  - [ ] 5.1 `environments/app-layer/quicksight/` 디렉토리 구조 생성
  ...
  - [ ] 5.2 `environments/app-layer/quicksight/main.tf` — Quick 계정 및 VPC Connection 생성
  ...
  - [ ] 5.3 `environments/app-layer/quicksight/s3.tf` — Quick 전용 S3 버킷 생성
  ...
  - [ ] 5.5 `environments/app-layer/quicksight/main.tf`에 Quick 서비스 역할 및 데이터 소스 등록 추가
  ...
  - [ ] 5.7 `environments/app-layer/quicksight/lambda.tf` — RAG Connector Lambda 생성
  ...
  - [ ] 5.8 `environments/app-layer/quicksight/monitoring.tf` — CloudWatch 알람 및 대시보드 생성
  ...
  - [ ] 5.11 RBAC Pipeline GROUP_MAP 확장 — Provisioner Lambda 수정
...
- [ ] 7. Lambda 소스 코드 — RAG Connector 구현
  ...
  - [ ] 7.3 `lambda/quick-rag-connector/handler.py`에 에러 처리 로직 추가
  ...
  - [ ] 7.4 `lambda/quick-rag-connector/requirements.txt` 생성
  ...
  - [ ]* 7.9 Lambda 단위 테스트 작성 — `lambda/quick-rag-connector/test_handler.py`
    - 응답 변환 엣지 케이스, 캐시 히트/미스, 에러 조건 검증
    - _요구사항: 5.2, 5.4, 5.5, 5.9_

- [ ] 8. 체크포인트 — Lambda 소스 코드 검증
  - `python -m pytest lambda/quick-rag-connector/test_handler.py` 실행하여 단위 테스트 통과 확인
  - 모든 테스트 통과 여부 확인. 문제가 있으면 사용자에게 질문하세요.

- [ ] 9. MCP Bridge — Quick 연동 도구 추가
  - [ ] 9.1 `mcp-bridge/server.mjs`에 `quick_dashboard_list` 도구 추가
  ...
  - [ ] 9.2 `mcp-bridge/server.mjs`에 `quick_dashboard_data` 도구 추가
...
- [ ] 10. OPA 정책 — `policies/quicksight-security.rego` 생성
  - [ ] 10.1 `deny_quicksight_sg_virginia_direct` 규칙 구현
    - Quick VPC Connection SG 아웃바운드 규칙에 Virginia CIDR(10.20.0.0/16) 직접 지정 시 deny
    - _요구사항: 4.9_

  - [ ] 10.2 `deny_quicksight_sg_open_egress` 규칙 구현
    - Quick 관련 SG에 0.0.0.0/0 아웃바운드 규칙 포함 시 deny
    - _요구사항: 7.5_

  - [ ] 10.3 `deny_quicksight_s3_public` 규칙 구현
